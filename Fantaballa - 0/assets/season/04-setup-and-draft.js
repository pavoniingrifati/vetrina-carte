...
function showSetup(){
 ...
 const competitionChoiceHtml=isFantacampionato?`<div class="season-mode-grid season-competition-grid"><button type="button" class="season-mode-btn ${state.competitionVariant!=='legend'?'active':''}" data-competition-variant="serie-a"><b>🇮🇹 Serie A</b><small><strong>20 club della stagione 2026/2027</strong> e 494 calciatori reali. È il Fantacampionato che hai già utilizzato.</small></button><button type="button" class="season-mode-btn ${state.competitionVariant==='legend'?'active':''}" data-competition-variant="legend"><b>🌟 Legend</b><small><strong>40 squadre storiche</strong> e 796 giocatori. Ogni run affronta 19 avversarie casuali, mentre il draft può pescare dall’intero database.</small></button></div>`:'';
 const modeIntroHtml=isFantacampionato?`<h2>Scegli il campionato</h2><p>Serie A mantiene il database attuale. Legend apre il draft con 40 squadre storiche; ogni campionato ne seleziona casualmente 19 come avversarie.</p>`:`<h2>Scegli la modalità</h2><p>Decidi se gli eventi devono coinvolgere soltanto la tua squadra oppure anche tutte le avversarie.</p>`;
 ...
 document.querySelectorAll('[data-competition-variant]').forEach(button=>button.onclick=()=>{const selected=normalizeCompetitionVariant(button.dataset.competitionVariant);if(selected!==state.competitionVariant){resetSetupForCompetitionVariant(selected);save();showSetup();setupScrollTo('setupModeStep');toast(selected==='legend'?'Database Legend caricato: 40 squadre storiche, 19 avversarie casuali per run.':'Database Serie A caricato.')}else{applyCompetitionVariantData(selected);updateCompetitionChrome()}});

...
function talentScoutDraftAdviceEnabled(){return coachIs('talent-scout')}
function draftBestCandidateId(){
 if(!talentScoutDraftAdviceEnabled())return'';
...
function renderDraftSelectionPreview(){
 ...
 const showAdvice=talentScoutDraftAdviceEnabled();
 const destination=showAdvice&&bestSlot?` · Slot consigliato ${esc(bestSlot.code)}`:(impact.mode==='bench'?' · Destinazione panchina':'');
 return `<section class="season-draft-selection-preview" aria-live="polite"><div class="season-draft-selection-player">${renderMiniAvatar(player,'small')}<div><span>Giocatore selezionato</span><b>${esc(player.name)} · OVR ${esc(player.ovr)}</b><small>${esc(player.Position)}${destination}</small></div></div><div class="season-draft-impact"><span><small>OVR squadra</small><b>${overallBefore} → ${overallAfter}</b></span><span><small>Intesa</small><b>${impact.chemBefore} → ${impact.chemAfter}</b></span><span><small>Bonus giocatore</small><b>${impact.mode==='starter'?`${formatSignedIntesa(impact.playerChem)} INT`:'PAN'}</b></span></div>${freeBench?`<button id="draftBenchQuickBtn" class="season-draft-quick-bench" type="button">Metti in panchina</button>`:''}</section>`;
}
function renderDraftAnalysis(chem=draftChemistry()){
 const starters=starterEntries().length,bench=benchEntries().length,effective=draftEffectiveAverageOvr(chem),needs=draftNeeds(),showAdvice=talentScoutDraftAdviceEnabled();
 const warnings=showAdvice?draftAnalysisWarnings(chem):[],bestId=showAdvice?draftBestCandidateId():'',best=playerById(bestId),bestImpact=best?draftCandidateImpact(best):null;
 ...
 const recommendation=showAdvice&&best&&bestImpact?`<section class="season-analysis-block season-analysis-recommendation"><span>Scelta consigliata dal Talent scout</span><b>${esc(best.name)} · ${esc(best.Position)}</b><small>${bestImpact.mode==='starter'?`OVR squadra ${starters?bestImpact.overallBefore.toFixed(1):'—'} → ${bestImpact.overallAfter.toFixed(1)} · Intesa ${bestImpact.chemBefore} → ${bestImpact.chemAfter}`:`Migliore opzione disponibile per la panchina · OVR ${esc(best.ovr)}`}</small></section>`:'';
 const indications=showAdvice?`<section class="season-analysis-block"><h3>Consigli del Talent scout</h3><ul>${warnings.map(text=>`<li>${esc(text)}</li>`).join('')}</ul></section>`:'';
 return `<div class="season-draft-analysis"><div class="season-analysis-title"><div><span>Analisi rosa</span><b>${draftComplete()?'Pronta per il campionato':'Cosa manca alla squadra'}</b></div><span class="season-analysis-score">${starters?effective.toFixed(1):'—'}</span></div><div class="season-analysis-metrics"><div><span>Intesa</span><b>${chem.score}/100</b></div><div><span>Titolari</span><b>${starters}/11</b></div><div><span>Panchina</span><b>${bench}/3</b></div><div><span>Bonus INT</span><b>${formatSignedIntesa(chem.totalBonus)}</b></div></div><section class="season-analysis-block"><h3>Posizioni mancanti</h3><div class="season-analysis-needs">${needMarkup}</div></section>${recommendation}${indications}</div>`;
}
...
function showSetup(){
 ...
 document.querySelectorAll('[data-competition-variant]').forEach(button=>button.onclick=()=>{const selected=normalizeCompetitionVariant(button.dataset.competitionVariant);if(selected!==state.competitionVariant){resetSetupForCompetitionVariant(selected);save();showSetup();setupScrollTo('setupModeStep');toast(selected==='champions'?'Database Champions League caricato: 36 squadre, quattro fasce e 970 giocatori.':selected==='legend'?'Database Legend caricato: 40 squadre storiche, 19 avversarie casuali per run.':'Database Serie A caricato.')}else{applyCompetitionVariantData(selected);updateCompetitionChrome()}});
...
async function playSeasonPackReveal(possibleClubIds,finalClubId,isReroll){
 modalRoot.innerHTML=`<div class="season-pack-reveal" id="seasonPackReveal"><div class="season-pack-stage" role="dialog" aria-live="assertive" aria-label="Apertura pacchetto club campionato"><div class="season-pack-kicker">${esc(state.competitionVariant==='champions'?'Champions League Pack':state.competitionVariant==='legend'?'Legend Pack':SEASON_CONFIG.labels.packKicker)}</div><div class="season-pack-scene" aria-hidden="true"><div class="season-pack-burst"></div><div class="season-pack-card-stack"><div class="season-pack-card-back"></div><div class="season-pack-card-back"></div><div class="season-pack-card-back"></div></div><div class="season-pack-nation-card" id="seasonPackNationCard"><div class="season-pack-card-label" id="seasonPackCardLabel">${isReroll?'Second chance':'Club draft'}</div><div class="season-pack-code" id="seasonPackCode">---</div><div class="season-pack-name" id="seasonPackNation">???</div></div><div class="season-pack-envelope" id="seasonPackEnvelope"><div class="season-pack-lid"></div><div class="season-pack-tear"></div><div class="season-pack-title-badge">FANTABALLA</div><div class="season-pack-seal">⚽</div></div></div><div class="season-pack-caption" id="seasonPackCaption">Scarta il pacchetto...</div></div></div>`;
...
function scrollDraftIntoView(selector,block='center'){
 requestAnimationFrame(()=>requestAnimationFrame(()=>{
   const target=document.querySelector(selector);
   if(target)target.scrollIntoView({behavior:'smooth',block,inline:'nearest'});
 }));
}
function selectDraftCandidate(id){
 if(draftRolling||draftComplete())return;
 const p=playerById(id);
 if(!draftPlayerIsValid(p))return toast('Giocatore non compatibile con gli slot rimasti.');
 const selecting=state.draft.pendingPlayerId!==String(id);
 state.draft.pendingPlayerId=selecting?String(id):'';
 if(selecting)mobileDraftTab='field';
 save();
 render();
 if(selecting)scrollDraftIntoView('.season-pitch-wrap','center');
}
...
function finishDraftPlacement(showPitchAnimation=false,placed=null){
 ...
 mobileDraftTab=complete?'roster':'field';
 save();
 render();
 if(!complete)scrollDraftIntoView('.season-draft-main-top','center');
...
function hasDiamondOvr(ovr){return ovrTierKey(ovr)==='diamond'}
function ovrTierClass(ovr){return `ovr-tier-${ovrTierKey(ovr)}`}
...
function draftCandidateOvrStyle(ovr){
 ...
 const tier=ovrTierKey(raw);
 const ovrBg=tier==='diamond'?'linear-gradient(180deg,#dffcff,#81ecff)':tier==='emerald'?'linear-gradient(180deg,#ddffe6,#52d98c)':tier==='purple'?'linear-gradient(180deg,#d8c3ff,#8f62d4)':tier==='gold'?'linear-gradient(180deg,#fff0a2,#eebc28)':tier==='blue'?'linear-gradient(180deg,#bfe9ff,#5ebdff)':(dark?'#ffe96c':'#10243a');
 const ovrText=tier==='diamond'?'#08344d':tier==='emerald'?'#0f4428':tier==='purple'?'#27143f':tier==='gold'?'#352100':tier==='blue'?'#08304a':(dark?'#10243a':'#ffffff');
 const ovrBorder=tier==='diamond'?'rgba(223,252,255,.95)':tier==='emerald'?'#eaffef':tier==='purple'?'#efe2ff':tier==='gold'?'#fff7d0':tier==='blue'?'#e5f6ff':(dark?'rgba(16,36,58,.2)':'rgba(255,255,255,.28)');
...
function renderDraftCandidates(){
 ...
 const showAdvice=talentScoutDraftAdviceEnabled(),bestId=showAdvice?draftBestCandidateId():'';
 return candidates.map(p=>{const rawOvr=Math.max(0,Number(p.ovr)||0),sub=isSubscriber(p),creator=isCreator(p),impact=draftCandidateImpact(p),rarity=draftRarityMeta(p.ovr),ovrTier=ovrTierClass(p.ovr),recommended=showAdvice&&String(p.id)===bestId;return `<button type="button" class="season-candidate rarity-${rarity.key} ${ovrTier} ${sub?'subscriber':''} ${creator?'creator':''} ${recommended?'recommended':''} ${state.draft.pendingPlayerId===String(p.id)?'active':''}" data-candidate="${esc(p.id)}" data-ovr="${rawOvr}" style="${draftCandidateOvrStyle(p.ovr)}" aria-label="${esc(p.name)}, overall ${esc(p.ovr)}${recommended?', scelta consigliata dal Talent scout':''}">${renderMiniAvatar(p)}<span class="season-candidate-body">${recommended?'<span class="season-candidate-best">Migliore scelta</span>':''}<span class="season-candidate-name">${esc(p.name)}</span><span class="season-candidate-meta"><span class="season-chip position">${esc(p.Position)}</span><span class="season-rarity-badge">${esc(rarity.label)}</span>${sub?'<span class="season-chip sub">ABBONATO</span>':''}${creator?'<span class="season-chip creator">CREATOR</span>':''}<span class="season-chip chemistry ${impact.mode==='bench'?'bench':''}">${impact.mode==='starter'?`${formatSignedIntesa(impact.playerChem)} INT`:'PAN'}</span></span></span><span class="season-chip ovr ${ovrTier}"><small>OVR</small>${esc(p.ovr)}</span></button>`}).join('')
}
...
function renderDraftTopPanel(hasPack,rollDisabled,rollText,total,currentDrawClub,drawStyle,draftRerollLimit){
 return `<div class="season-draft-main-top"><div class="season-draw-card" style="${drawStyle}"><div class="season-draw-head"><span class="season-draw-mini">Club estratto</span><span class="season-draw-pick">Pick ${String(Math.min(14,total+1)).padStart(2,'0')}</span></div><div class="season-draw-row"><div class="season-draw-copy"><div class="season-draw-nation" title="${currentDrawClub?esc(currentDrawClub.name):'Apri il pack'}">${currentDrawClub?esc(currentDrawClub.name):'Nessun club'}</div></div></div></div><div class="season-roll-actions"><button id="draftRollBtn" class="season-roll-btn" ${rollDisabled?'disabled':''}>${rollText}</button></div><div class="season-reroll-note ${hasPack?'active-reroll':'first-roll'}">${hasPack?(state.draft.rerolls>0?`Puoi cambiare il club ancora ${state.draft.rerolls} volte su ${draftRerollLimit}. `:'Hai terminato i re-roll. '):'Apri il primo pack per iniziare. '}Le riserve possono essere scelte in qualsiasi momento.</div></div>`
}
 ...
 screen.innerHTML=`<div class="season-draft-page">
   ...
   <div class="season-draft-shell">
     ...
     <main id="seasonFieldPane" aria-labelledby="seasonMobileField" class="season-draft-main season-mobile-pane ${mobileDraftTab==='field'?'mobile-active':''}" data-mobile-pane="field" role="tabpanel">
       ${renderDraftTopPanel(hasPack,rollDisabled,rollText,total,currentDrawClub,drawStyle,draftRerollLimit)}
       ${renderSeasonPitch()}
     </main>

...
function playRound(mode='instant'){
 ...
 const finishRound=()=>{
   ...
   modalRoot.innerHTML=`<div class="modal-backdrop"><section class="modal" role="dialog" aria-modal="true"><div class="label">Rokky · fine partita</div><h2>Vuoi provare ad annullare un gol?</h2><p>Puoi tentare di cancellare un gol avversario dal risultato finale.</p><div class="goal-line"><b>80%</b> Il gol viene annullato.<br><b>20%</b> Vieni scoperto e perdi tutti i punti in classifica.</div><div class="top-actions"><button id="rokkyAttempt" class="btn red" type="button">Tenta l’annullamento</button><button id="rokkyKeepResult" class="btn primary" type="button">Mantieni il risultato</button></div></section></div>`;
   document.getElementById('rokkyKeepResult').onclick=()=>{rokkyOutcome={attempted:false,success:false,caught:false};commitRound()};
   document.getElementById('rokkyAttempt').onclick=()=>{if(Math.random()<.8)applyRokkyGoalCancellation();else rokkyOutcome={attempted:true,success:false,caught:true,originalGoalsAgainst:Number(officialGoalsAgainst)||0,finalGoalsAgainst:Number(officialGoalsAgainst)||0};commitRound()};
 };

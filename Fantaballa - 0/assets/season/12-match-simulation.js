...
function postMatch(){
 ...
 const figcInjuriesApplied=applyPendingFigcUserInjuries(result),risks=state.activeEffects.filter(effect=>effect.type==='injuryRisk');
 ...
 if(!figcInjuriesApplied&&!matchWasFormulaOneWalkover&&state.seasonRules.longMatchRisk&&Number(result?.matchDuration)===120&&Math.random()<physioAdjustedInjuryChance(.55)){const entry=randomOwnEntry();if(entry){setOwnPlayerInjury(entry,2);result.longMatchInjury={playerId:String(entry.playerId),name:entry.player.name,duration:2};}}

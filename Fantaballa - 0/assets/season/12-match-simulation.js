...
function playRound(mode='instant'){
 ...
 const opponent=teamById(fixture.home===USER_ID?fixture.away:fixture.home),bottomHalfOpponentRankBefore=bottomHalfUnbeatenOpponentRank(opponent?.id),questOpponentTop3Before=questOpponentIsTopThree(opponent?.id),lineup=resolveLineup(),opponentLineup=teamMatchLineup(opponent),baseUserPower=matchPower(),userHome=fixture.home===USER_ID,hungerWalkover=!error404Corrupted&&isTeamEliminated(opponent?.id);
 ...
 const opponentPower=Math.max(1,opponentMatchPower(opponent)+opponentEventOvr+deathMatchBonus+rokkyOpponentBonus-(redCandidate?10:0)-fantaballopoliRisks.powerPenalty),userPower=Math.max(35,baseUserPower-(ownRedEntry?10:0));
 ...
 if(bottomHalfOpponentRankBefore>=11&&state.seasonRules?.bottomHalfUnbeaten&&Number(officialGoalsFor)<Number(officialGoalsAgainst)){
 ...
 const finishRound=()=>{
   if(committed)return;
   const canUseRokky=coachIs('rokky')&&!rokkyDecisionShown&&!formulaOneWalkoverView&&!error404Corrupted&&!hungerWalkover&&Number(officialGoalsAgainst)>0;

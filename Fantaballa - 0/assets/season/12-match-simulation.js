...
function playFgciLeaderRestRound(mode,fixture,leaderId){
 ...
 currentRound().forEach(match=>{
  ...
  const homeLineup=teamMatchLineup(home),awayLineup=teamMatchLineup(away),homeEvents=buildTeamGoals(homeScore,homeLineup,home,away,[],90),awayEvents=buildTeamGoals(awayScore,awayLineup,away,home,[],90);
  updateStanding(homeId,homeScore,awayScore);updateStanding(awayId,awayScore,homeScore);
  ...
  roundResults.push({matchday:state.matchday+1,homeId,awayId,homeName:home?.name||'',awayName:away?.name||'',homeScore,awayScore,homeGoals:homeEvents,awayGoals:awayEvents,mvpId:awards.mvpId,mvpScore:awards.mvpScore,mvpTeamId:awards.mvpTeamId});
 });

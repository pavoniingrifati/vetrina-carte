...
function updateStanding(id,gf,ga,outcome={}){
 const s=state.standings[id];if(!s)return;const officialGf=Number(gf)||0,officialGa=Number(ga)||0,winnerId=String(outcome?.winnerId||''),resolved=matchOutcomeScores(officialGf,officialGa,winnerId,id);s.p++;s.gf+=officialGf;s.ga+=officialGa;if(resolved.gf>resolved.ga)s.w++;else if(resolved.gf===resolved.ga)s.d++;else s.l++;if(isChampionsCompetition()&&outcome?.away){s.awayGoals=Math.max(0,Number(s.awayGoals)||0)+officialGf;if(resolved.gf>resolved.ga)s.awayWins=Math.max(0,Number(s.awayWins)||0)+1;}
...
function playLiveMatch({commentary,homeTeam,awayTeam,homeGoals,awayGoals,matchday,duration=90,startMinute=0,label=''},finishRound){
 ...
 modalRoot.innerHTML=`<div class="modal-backdrop"><section class="modal live-match-modal" role="dialog" aria-modal="true" aria-labelledby="liveMatchTitle"><header class="live-match-head"><div class="live-match-headline"><div><div class="label">${esc(label||`Giornata ${matchday}`)}</div><b id="liveMatchTitle">Cronaca della partita</b></div><span class="live-status" aria-label="Partita in diretta">Live</span></div></header><div class="live-scoreboard" role="status" aria-live="polite" aria-atomic="true"><div class="live-team" style="${teamCssVars(homeTeam)}"><span class="live-team-colors"></span><b title="${esc(homeTeam.name)}">${esc(homeTeam.name)}</b></div><div class="live-score"><span id="liveHomeScore">0</span><em>–</em><span id="liveAwayScore">0</span></div><div class="live-team" style="${teamCssVars(awayTeam)}"><span class="live-team-colors"></span><b title="${esc(awayTeam.name)}">${esc(awayTeam.name)}</b></div></div><div class="live-clock-wrap"><span id="liveClock" class="live-clock">0'</span><div class="live-progress"><i id="liveProgress"></i></div></div><div id="liveCommentary" class="live-commentary"><div class="live-empty">La partita sta per iniziare…</div></div><footer class="live-match-controls"><button id="pauseLiveMatch" class="btn secondary-strong" type="button">⏸ Pausa</button><button id="skipLiveMatch" class="btn match-instant-button" type="button" data-single-action data-busy-announcement="Simulazione del resto della partita avviata.">⚡ Simula il resto</button></footer></section></div>`;
...
function figcInjuryProtected(playerId){
 if(parallelCupDisciplineImmunity())return true;
 if(state.activeEffects.some(effect=>effect.type==='injuryImmunity'))return true;
 if(typeof redPillProtectsPlayer==='function'&&redPillProtectsPlayer(playerId))return true;
 return false;
}
function prepareFigcNegativeUserInjuries(matchMinutes,formulaOneWalkover=false){
 const pending={eventInjuries:[],longMatchInjury:null};
 if(!figcNegativeIncidentRuleActive()||formulaOneWalkover)return pending;
 const used=new Set(),available=()=>shuffle(rosterPlayers().filter(entry=>entry&&entry.player&&!used.has(String(entry.playerId))&&!figcInjuryProtected(entry.playerId)));
 const risks=state.activeEffects.filter(effect=>effect.type==='injuryRisk');
 if(risks.length){
  const chance=physioAdjustedInjuryChance(1-risks.reduce((remaining,effect)=>remaining*(1-clamp(Number(effect.chance)||0,0,1)),1));
  if(Math.random()<chance){
   const count=Math.max(1,...risks.map(effect=>Number(effect.count)||1)),duration=Math.max(1,...risks.map(effect=>Number(effect.duration)||1));
   available().slice(0,count).forEach(entry=>{used.add(String(entry.playerId));pending.eventInjuries.push({playerId:String(entry.playerId),name:entry.player.name,duration})});
  }
 }
 if(state.seasonRules.longMatchRisk&&Number(matchMinutes)===120&&Math.random()<physioAdjustedInjuryChance(.55)){
  const entry=available()[0];if(entry){used.add(String(entry.playerId));pending.longMatchInjury={playerId:String(entry.playerId),name:entry.player.name,duration:2};}
 }
 return pending;
}
function applyPendingFigcUserInjuries(result){
 if(!result?.figcPendingInjuries)return false;
 const pending=result.figcPendingInjuries,applied=[];
 (Array.isArray(pending.eventInjuries)?pending.eventInjuries:[]).forEach(item=>{const entry=rosterEntry(item.playerId);if(entry&&setOwnPlayerInjury(entry,item.duration))applied.push({...item})});
 result.eventInjuries=applied;
 const long=pending.longMatchInjury,entry=long?rosterEntry(long.playerId):null;
 if(entry&&setOwnPlayerInjury(entry,long.duration))result.longMatchInjury={...long};else result.longMatchInjury=null;
 result.figcPendingInjuries=null;
 return true;
}


function extremeVarRuleMode(){const mode=String(state.seasonRules?.extremeVarRule||'');return mode==='first'||mode==='last'?mode:''}
function removeWholeGoalEvent(events,index){
 const event=events[index];if(!event)return null;
 const removed={...event,goalValue:Math.max(1,Number(event.goalValue)||1)};events.splice(index,1);return removed;
}
function applyExtremeVarToGoalEvents(userEvents,opponentEvents){
 const mode=extremeVarRuleMode();if(!mode)return null;
 const all=[];
 (Array.isArray(userEvents)?userEvents:[]).forEach((event,index)=>all.push({side:'user',events:userEvents,index,event}));
 (Array.isArray(opponentEvents)?opponentEvents:[]).forEach((event,index)=>all.push({side:'opponent',events:opponentEvents,index,event}));
 if(!all.length)return null;
 all.sort((a,b)=>(Number(a.event?.minute)||0)-(Number(b.event?.minute)||0)||(a.side==='user'?-1:1));
 const chosen=mode==='last'?all[all.length-1]:all[0],removed=removeWholeGoalEvent(chosen.events,chosen.index);
 if(!removed)return null;
 return{mode,side:chosen.side,minute:Number(removed.minute)||0,player:String(removed.player||'un giocatore'),value:Math.max(1,Number(removed.goalValue)||1)};
}

function fgciDirectMatchRule(){const rule=String(state.seasonRules?.fgciDirectMatchRule||'');return rule==='penalty-lottery'||rule==='effective-time'?rule:''}
function fgciPenaltyLotteryActive(){return fgciDirectMatchRule()==='penalty-lottery'}
function fgciEffectiveTimeActive(){return fgciDirectMatchRule()==='effective-time'}
function effectiveTimeOpponentInjury(team,lineup){
 if(!fgciEffectiveTimeActive()||!team||Math.random()>=.62)return null;
 const entry=pick((Array.isArray(lineup)?lineup:[]).filter(item=>item?.player));if(!entry)return null;
 const duration=1+Math.floor(Math.random()*3),status=opponentStatusOf(team,entry.playerId);status.injury=Math.max(Number(status.injury)||0,duration);
 return{playerId:String(entry.playerId),playerName:String(entry.player?.name||'Giocatore'),teamName:String(team.name||'Avversari'),duration};
}
function wweWatchOutActive(){return String(state.seasonRules?.wweRule||'')==='watch-out'}
function wweIncidentPlayer(candidate,lineup=[]){
 const raw=candidate?.player||candidate||{},id=String(candidate?.playerId||raw?.id||'');
 const found=(Array.isArray(lineup)?lineup:[]).find(entry=>String(entry?.playerId||entry?.player?.id||'')===id),player=found?.player||raw;
 return id?{playerId:id,playerName:String(player?.name||candidate?.name||'Giocatore'),player}:null;
}
function wweRandomIncidentPlayer(lineup=[],used=new Set()){
 const available=(Array.isArray(lineup)?lineup:[]).filter(entry=>entry?.player&&!used.has(String(entry.playerId||entry.player.id||'')));
 const chosen=pick(available);return chosen?wweIncidentPlayer(chosen,lineup):null;
}
function wweApplyIncidentStatus(team,incident,isUser){
 if(!incident?.playerId||!incident.generated)return;
 if(isUser){const status=statusOf(incident.playerId);if(incident.type==='red')status.suspension=Math.max(Number(status.suspension)||0,2);else status.injury=Math.max(Number(status.injury)||0,(Number(incident.duration)||1)+1);}
 else{const status=opponentStatusOf(team,incident.playerId);if(incident.type==='red')status.suspension=Math.max(Number(status.suspension)||0,2);else status.injury=Math.max(Number(status.injury)||0,(Number(incident.duration)||1)+1);}
}
function wweTeamIncidents({team,lineup=[],reds=[],injuries=[],isUser=false,randomIncidents=true}={}){
 const incidents=[],seen=new Set(),add=(candidate,type,generated=false,duration=1)=>{const info=wweIncidentPlayer(candidate,lineup);if(!info)return;const key=`${type}:${info.playerId}`;if(seen.has(key))return;seen.add(key);const incident={...info,type,generated,duration:Math.max(1,Number(duration)||1)};incidents.push(incident);wweApplyIncidentStatus(team,incident,isUser);};
 (Array.isArray(reds)?reds:[]).forEach(item=>add(item,'red',false,1));
 (Array.isArray(injuries)?injuries:[]).forEach(item=>add(item,'injury',false,Number(item?.duration)||1));
 if(randomIncidents){const used=new Set(incidents.map(item=>item.playerId));if(Math.random()<.12){const player=wweRandomIncidentPlayer(lineup,used);if(player){used.add(player.playerId);add(player,'red',true,1)}}if(Math.random()<.025){const player=wweRandomIncidentPlayer(lineup,used);if(player){used.add(player.playerId);add(player,'red',true,1)}}if(Math.random()<.20){const player=wweRandomIncidentPlayer(lineup,used);if(player){used.add(player.playerId);add(player,'injury',true,1+Math.floor(Math.random()*2))}}if(Math.random()<.04){const player=wweRandomIncidentPlayer(lineup,used);if(player){used.add(player.playerId);add(player,'injury',true,1+Math.floor(Math.random()*2))}}}
 return incidents;
}
function prepareWweRiskInjuries(pending,lineup=[]){
 if(!wweWatchOutActive()||!pending)return pending;
 pending.eventInjuries=Array.isArray(pending.eventInjuries)?pending.eventInjuries:[];
 if(pending.eventInjuries.length||pending.longMatchInjury)return pending;
 const risks=state.activeEffects.filter(effect=>effect.type==='injuryRisk');if(!risks.length)return pending;
 const chance=physioAdjustedInjuryChance(1-risks.reduce((remaining,effect)=>remaining*(1-clamp(Number(effect.chance)||0,0,1)),1));if(Math.random()>=chance)return pending;
 const count=Math.max(1,...risks.map(effect=>Number(effect.count)||1)),duration=Math.max(1,...risks.map(effect=>Number(effect.duration)||1)),candidates=shuffle((Array.isArray(lineup)?lineup:[]).filter(entry=>entry?.player)).slice(0,count);
 pending.eventInjuries.push(...candidates.map(entry=>({playerId:String(entry.playerId||entry.player.id||''),name:String(entry.player?.name||'Giocatore'),duration,wweRisk:true})).filter(item=>item.playerId));return pending;
}
function wweIncidentGoalEvent(incident,team,opponent,duration=90,index=0){
 const total=Math.max(30,Number(duration)||90),minute=Math.min(total,8+index*Math.max(5,Math.floor((total-12)/4)));
 return{minute,playerId:String(incident.playerId||''),assistId:'',player:String(incident.playerName||'Giocatore'),assist:'',teamId:String(team?.id||''),teamName:String(team?.name||''),goalValue:1,isWweIncidentGoal:true,wweIncidentType:String(incident.type||''),description:`Watch out!: ${incident.type==='red'?'l’espulsione':'l’infortunio'} di ${incident.playerName||'un giocatore'} vale un gol per ${team?.name||'la squadra'}.`};
}
function applyWweWatchOutToMatch({teamA,teamB,lineupA=[],lineupB=[],eventsA=[],eventsB=[],redsA=[],redsB=[],injuriesA=[],injuriesB=[],duration=90,userSideA=false,userSideB=false,randomIncidents=true}={}){
 if(!wweWatchOutActive())return null;
 const incidentsA=wweTeamIncidents({team:teamA,lineup:lineupA,reds:redsA,injuries:injuriesA,isUser:userSideA,randomIncidents}),incidentsB=wweTeamIncidents({team:teamB,lineup:lineupB,reds:redsB,injuries:injuriesB,isUser:userSideB,randomIncidents});
 eventsA.splice(0,eventsA.length,...incidentsA.map((incident,index)=>wweIncidentGoalEvent(incident,teamA,teamB,duration,index)));eventsB.splice(0,eventsB.length,...incidentsB.map((incident,index)=>wweIncidentGoalEvent(incident,teamB,teamA,duration,index)));
 return{scoreA:incidentsA.length,scoreB:incidentsB.length,incidentsA:incidentsA.map(({playerId,playerName,type,duration,generated})=>({playerId,playerName,type,duration,generated})),incidentsB:incidentsB.map(({playerId,playerName,type,duration,generated})=>({playerId,playerName,type,duration,generated}))};
}

function showRoyalRumble(){
 const rr=typeof royalRumbleState==='function'?royalRumbleState():null;if(!rr||!rr.active){state.phase=rr?.completed?'finished':'season';save();render();return}
 const queue=Array.isArray(rr.queue)?rr.queue:[],index=Math.max(0,Number(rr.index)||0),opponent=teamById(queue[index]);if(!opponent){rr.active=false;rr.completed=true;rr.champion=true;rr.eliminated=false;state.phase='finished';save();render();return}
 const resultRows=(Array.isArray(rr.results)?rr.results:[]).slice(-5).reverse().map(item=>`<div class="goal-line"><b>${esc(item.opponentName||'Avversario')}</b> · ${Number(item.gf)||0}-${Number(item.ga)||0}${item.penalties?` · rigori ${Number(item.penalties.for)||0}-${Number(item.penalties.against)||0}`:''}</div>`).join('');
 screen.innerHTML=`<section class="panel season-finished-view royal-rumble-view"><div class="final-hero"><div class="label">🤼 Arriva la WWE!</div><h2>Royal Rumble</h2><div class="final-position">${index+1}/${queue.length}</div><p>Tutte le sfide si disputano nella stessa giornata. I punti restano fermi a <b>${Number(rr.startingPoints)||0}</b>.</p></div><section class="panel"><div class="label">Prossimo ingresso</div><h3>${esc(state.teamName)} vs ${esc(opponent.name)}</h3><p>Devi vincere per continuare. In caso di parità si procede con supplementari e rigori.</p><button id="playRoyalRumble" class="btn primary" type="button">Affronta ${esc(opponent.name)}</button></section>${resultRows?`<section class="panel"><h3>Ultime eliminazioni</h3>${resultRows}</section>`:''}</section>`;
 document.getElementById('playRoyalRumble').onclick=()=>playRoyalRumbleMatch();
}
function playRoyalRumbleMatch(){
 const rr=typeof royalRumbleState==='function'?royalRumbleState():null;if(!rr?.active)return;const queue=Array.isArray(rr.queue)?rr.queue:[],index=Math.max(0,Number(rr.index)||0),opponent=teamById(queue[index]);if(!opponent)return;
 const userTeam=teamById(USER_ID)||{id:USER_ID,name:state.teamName},userLineup=resolveLineup(),opponentLineup=teamMatchLineup(opponent),userPower=Math.max(35,matchPower()),opponentPower=Math.max(35,opponentMatchPower(opponent)),userHome=index%2===0;
 let [homeBase,awayBase]=simulateScore(userHome?userPower:opponentPower,userHome?opponentPower:userPower,.12,90),gf=userHome?homeBase:awayBase,ga=userHome?awayBase:homeBase;
 const userEvents=buildTeamGoals(gf,userLineup,userTeam,opponent,[],90),opponentEvents=buildTeamGoals(ga,opponentLineup,opponent,userTeam,[],90),resolved=resolveNoDrawMatch({scoreA:gf,scoreB:ga,eventsA:userEvents,eventsB:opponentEvents,lineupA:userLineup,lineupB:opponentLineup,teamA:userTeam,teamB:opponent,powerA:userPower,powerB:opponentPower,duration:90,force:true});
 gf=Number(resolved.scoreA)||0;ga=Number(resolved.scoreB)||0;const won=String(resolved.winnerId||'')===String(USER_ID),awards=recordLeagueMatchPlayerStats({homeTeam:userTeam,awayTeam:opponent,homeLineup:userLineup,awayLineup:opponentLineup,homeScore:gf,awayScore:ga,homeEvents:userEvents,awayEvents:opponentEvents}),penalties=resolved.penalties?{for:Number(resolved.penalties.home)||0,against:Number(resolved.penalties.away)||0,winnerId:String(resolved.penalties.winnerId||'')}:null,result={opponentId:String(opponent.id),opponentName:String(opponent.name),gf,ga,won,extraTime:Boolean(resolved.extraTime),penalties,winnerId:String(resolved.winnerId||'')};
 rr.results=Array.isArray(rr.results)?rr.results:[];rr.results.push(result);rr.index=index+1;state.matchday=Math.max(Number(state.matchday)||0,Number(rr.singleMatchday)||Number(state.matchday)||0);
 state.history.push({matchday:Number(rr.singleMatchday)||state.matchday,opponent:opponent.name,opponentId:opponent.id,home:userHome,gf,ga,displayGf:gf,displayGa:ga,winnerId:String(resolved.winnerId||''),extraTime:Boolean(resolved.extraTime),penalties,goals:userEvents,opponentGoals:opponentEvents,highlights:[...userEvents,...opponentEvents].sort((a,b)=>a.minute-b.minute),commentary:[],royalRumble:true,royalRumbleIndex:index+1,pointsAwarded:0,pointsAdjustment:0,pointsNote:`Royal Rumble: punti invariati a ${Number(rr.startingPoints)||0}.`,mvpId:awards.mvpId||'',mvpScore:awards.mvpScore||0,mvpTeamId:awards.mvpTeamId||''});state.lastResult=state.history[state.history.length-1];
 const royalCellOutcome=typeof applyCellPerfectAfterMatch==='function'?applyCellPerfectAfterMatch(state.lastResult):null,royalCyborgRestored=typeof cyborgRestoreProtectedRoster==='function'?cyborgRestoreProtectedRoster():[];
 if(royalCyborgRestored.length){state.lastResult.eventUpdates=Array.isArray(state.lastResult.eventUpdates)?state.lastResult.eventUpdates:[];state.lastResult.eventUpdates.push({success:true,title:'Cyborg',message:`Neutralizzato un malus OVR su ${royalCyborgRestored.length} ${royalCyborgRestored.length===1?'giocatore':'giocatori'}.`})}
 if(!won){rr.active=false;rr.completed=true;rr.eliminated=true;rr.champion=false;state.seasonRules.eliminatedTeamIds=[...new Set([...(state.seasonRules.eliminatedTeamIds||[]).map(String),String(USER_ID)])];}else if(rr.index>=queue.length){rr.active=false;rr.completed=true;rr.champion=true;rr.eliminated=false;state.seasonRules.eliminatedTeamIds=(state.seasonRules.eliminatedTeamIds||[]).filter(id=>String(id)!==String(USER_ID));}
 save();const statusBase=rr.completed?(rr.champion?'Hai eliminato tutte le squadre: sei campione!':'Non hai vinto la sfida: sei eliminato dal campionato.'):`Vittoria! Restano ${Math.max(0,queue.length-rr.index)} avversarie.`,status=`${statusBase}${royalCellOutcome?.message?` ${royalCellOutcome.message}`:''}${royalCyborgRestored.length?` Cyborg ha ripristinato ${royalCyborgRestored.length} giocatori.`:''}`;
 modalRoot.innerHTML=`<div class="modal-backdrop"><div class="modal result-modal-expanded"><div class="label">🤼 Royal Rumble · sfida ${index+1}</div><div class="match-result"><div class="team-name">${esc(state.teamName)} vs ${esc(opponent.name)}</div><div class="score">${gf}–${ga}${penalties?` <small>(${penalties.for}–${penalties.against} d.c.r.)</small>`:''}</div><div class="goal-line ${won?'':'red-card-event'}"><b>${won?'✅ Avversaria eliminata':'❌ Eliminazione'}</b><br>${esc(status)} I punti restano ${Number(rr.startingPoints)||0}.</div><button id="closeRoyalRumble" class="btn primary result-continue-top">Continua</button></div></div></div>`;
 document.getElementById('closeRoyalRumble').onclick=()=>{modalRoot.innerHTML='';if(rr.completed)state.phase='finished';save();render()};
}

function applyHeroAcademiaFourOneAfterMatch(result){
 if(String(state.seasonRules?.heroAcademiaRule||'')!=='four-one'||!result||result.matchNotPlayed)return null;
 const gf=Number.isFinite(Number(result.displayGf))?Number(result.displayGf):Number(result.gf)||0;
 const ga=Number.isFinite(Number(result.displayGa))?Number(result.displayGa):Number(result.ga)||0;
 if(gf===4&&ga===1){
  const standing=userStanding(),lost=Math.max(0,Number(standing?.pts)||0);if(standing)standing.pts=0;
  const outcome={type:'win-reset',lostPoints:lost,message:`All 4 1 & 1 4 all: hai vinto 4-1 e perdi tutti i ${lost} punti in classifica.`};
  result.heroAcademiaOutcome=outcome;result.pointsAdjustment=(Number(result.pointsAdjustment)||0)-lost;result.pointsAwarded=(Number(result.pointsAwarded)||0)-lost;result.pointsNote=[result.pointsNote,outcome.message].filter(Boolean).join(' ');return outcome;
 }
 if(gf===1&&ga===4){
  let erased=0,teams=0;Object.entries(state.standings||{}).forEach(([id,standing])=>{if(String(id)===String(USER_ID))return;erased+=Math.max(0,Number(standing?.pts)||0);if(standing)standing.pts=0;teams++;});
  const outcome={type:'loss-reset-others',erasedPoints:erased,teams,message:`All 4 1 & 1 4 all: hai perso 1-4, ma le altre ${teams} squadre perdono complessivamente ${erased} punti. I tuoi punti restano invariati.`};
  result.heroAcademiaOutcome=outcome;result.pointsNote=[result.pointsNote,outcome.message].filter(Boolean).join(' ');return outcome;
 }
 return null;
}
function playFgciPenaltyLotteryRound(mode='instant'){
 if(state.pendingEvent&&!state.pendingEvent.resolved)return;
 const fixture=userFixture(),userHome=String(fixture.home)===String(USER_ID),opponentId=String(userHome?fixture.away:fixture.home),opponent=teamById(opponentId),userLineup=resolveLineup(),opponentLineup=teamMatchLineup(opponent),beforePts=Number(userStanding()?.pts)||0,roundResults=[],restLeaderId=currentFgciLeaderRestId();
 currentRound().forEach(match=>{
  const homeId=String(match.home),awayId=String(match.away),home=teamById(homeId),away=teamById(awayId);
  if(restLeaderId&&(homeId===restLeaderId||awayId===restLeaderId)){roundResults.push({matchday:state.matchday+1,homeId,awayId,homeName:home?.name||'',awayName:away?.name||'',rested:true,restLeaderId});return;}
  const homeLineup=homeId===String(USER_ID)?userLineup:teamMatchLineup(home),awayLineup=awayId===String(USER_ID)?userLineup:teamMatchLineup(away);
  const homePower=homeId===String(USER_ID)?Math.max(35,matchPower()):Math.max(35,opponentMatchPower(home));
  const awayPower=awayId===String(USER_ID)?Math.max(35,matchPower()):Math.max(35,opponentMatchPower(away));
  const shootout=simulatePenaltyShootout(homePower,awayPower),winnerId=String(shootout.scoreA>shootout.scoreB?homeId:awayId);
  updateStanding(homeId,0,0,{winnerId,away:false});updateStanding(awayId,0,0,{winnerId,away:true});
  const awards=recordLeagueMatchPlayerStats({homeTeam:home,awayTeam:away,homeLineup,awayLineup,homeScore:0,awayScore:0,homeEvents:[],awayEvents:[]});
  roundResults.push({matchday:state.matchday+1,homeId,awayId,homeName:home?.name||'',awayName:away?.name||'',homeScore:0,awayScore:0,winnerId,penaltyLottery:true,penalties:{home:Number(shootout.scoreA)||0,away:Number(shootout.scoreB)||0,winnerId},homeGoals:[],awayGoals:[],mvpId:awards.mvpId,mvpScore:awards.mvpScore,mvpTeamId:awards.mvpTeamId});
 });
 const userRound=roundResults.find(item=>String(item.homeId)===String(USER_ID)||String(item.awayId)===String(USER_ID));
 if(!userRound)return;
 const winnerId=String(userRound.winnerId||''),penFor=userHome?Number(userRound.penalties?.home)||0:Number(userRound.penalties?.away)||0,penAgainst=userHome?Number(userRound.penalties?.away)||0:Number(userRound.penalties?.home)||0;
 let afterPts=Number(userStanding()?.pts)||0;if(winnerId===String(USER_ID)&&state.seasonRules.marottaDoubleWins){const earned=Math.max(0,afterPts-beforePts);if(userStanding())userStanding().pts+=earned;afterPts+=earned;}
 const awarded=afterPts-beforePts,note=`Lotteria dei rigori: 0-0 regolamentare, ${penFor}-${penAgainst} ai rigori. ${winnerId===String(USER_ID)?`Vittoria e +${awarded} punti.`:`Sconfitta e ${awarded} punti.`}`;
 state.lastRoundResults=roundResults;
 state.history.push({matchday:state.matchday+1,opponent:opponent?.name||'Avversario',opponentId,home:userHome,gf:0,ga:0,displayGf:0,displayGa:0,winnerId,extraTime:false,penalties:{for:penFor,against:penAgainst,winnerId},noDrawRuleNote:note,penaltyLottery:true,goals:[],opponentGoals:[],highlights:[],commentary:[{minute:0,type:'penalties',icon:'🎯',title:'Lotteria dei rigori',text:`La partita inizia direttamente dal dischetto: ${penFor}-${penAgainst}.`,teamId:'',scoreAfter:'0–0'}],lineup:safeMatchLineup(userLineup),opponentLineup:safeMatchLineup(opponentLineup),cleanSheet:true,opponentCleanSheet:true,ownRedCard:false,opponentRedCard:false,pointsAwarded:awarded,pointsAdjustment:awarded,pointsNote:note,playMode:mode,matchDuration:0,fgciDirectMatchRule:'penalty-lottery'});
 state.lastResult=state.history[state.history.length-1];postMatch();save();showResultModal();
}

function currentFgciLeaderRestId(){return state.seasonRules?.fgciLeaderRestRule?String(sortedTable()[0]?.id||''):''}
function simulateSoulSecondLeg({userHome,userPower,opponentPower,lineup,opponentLineup,userTeam,opponent,matchMinutes,matchStartMinute}){
 let [homeBase,awayBase]=simulateScore(userHome?userPower:opponentPower,userHome?opponentPower:userPower,curvaContestHomeAdvantage(userHome),Math.max(1,matchMinutes-matchStartMinute));
 let gf=userHome?homeBase:awayBase,ga=userHome?awayBase:homeBase;
 const userEvents=buildTeamGoals(gf,lineup,userTeam,opponent,[],matchMinutes,matchStartMinute),opponentEvents=buildTeamGoals(ga,opponentLineup,opponent,userTeam,[],matchMinutes,matchStartMinute);
 normalizeGoalEventsToWindow(userEvents,matchStartMinute,matchMinutes);normalizeGoalEventsToWindow(opponentEvents,matchStartMinute,matchMinutes);
 const varOutcome=applyExtremeVarToGoalEvents(userEvents,opponentEvents);
 gf=scoreGoalEvents(userEvents);ga=scoreGoalEvents(opponentEvents);
...
function playFgciLeaderRestRound(mode,fixture,leaderId){
 const roundResults=[],sharedPreRound=sharedRegulationPreRoundContext();
 currentRound().forEach(match=>{
  const homeId=String(match.home),awayId=String(match.away),home=teamById(homeId),away=teamById(awayId);
  if(homeId===leaderId||awayId===leaderId){roundResults.push({matchday:state.matchday+1,homeId,awayId,homeName:home?.name||'',awayName:away?.name||'',rested:true,restLeaderId:leaderId});return;}
  const homePower=opponentMatchPower(home),awayPower=opponentMatchPower(away);let [homeScore,awayScore]=simulateScore(homePower,awayPower,.18,90);
  if(chaosEnabled())[homeScore,awayScore]=applyChaosScoreRules(home,away,homeScore,awayScore);
  const homeLineup=teamMatchLineup(home),awayLineup=teamMatchLineup(away),homeEvents=buildTeamGoals(homeScore,homeLineup,home,away,[],90),awayEvents=buildTeamGoals(awayScore,awayLineup,away,home,[],90),scoreOutcome=typeof sharedRegulationScoreEffects==='function'?sharedRegulationScoreEffects({homeTeam:home,awayTeam:away,homeEvents,awayEvents,homeScore,awayScore,homePower,awayPower,duration:90}):{homeScore,awayScore,notes:[]};homeScore=scoreOutcome.homeScore;awayScore=scoreOutcome.awayScore;
  const beforePoints={home:Number(state.standings?.[homeId]?.pts)||0,away:Number(state.standings?.[awayId]?.pts)||0},winnerId=homeScore>awayScore?homeId:awayScore>homeScore?awayId:'';updateStanding(homeId,homeScore,awayScore,{winnerId,away:false});updateStanding(awayId,awayScore,homeScore,{winnerId,away:true});const sharedPoints=applySharedRegulationPointsToMatch({homeId,awayId,homeScore,awayScore,winnerId,homeGoals:homeEvents,awayGoals:awayEvents,preRoundContext:sharedPreRound,beforePoints});
  const awards=recordLeagueMatchPlayerStats({homeTeam:home,awayTeam:away,homeLineup,awayLineup,homeScore,awayScore,homeEvents,awayEvents});
  roundResults.push({matchday:state.matchday+1,homeId,awayId,homeName:home?.name||'',awayName:away?.name||'',homeScore,awayScore,homeGoals:homeEvents,awayGoals:awayEvents,mvpId:awards.mvpId,mvpScore:awards.mvpScore,mvpTeamId:awards.mvpTeamId,sharedRegulationNotes:[...(scoreOutcome.notes||[]),...(sharedPoints.notes||[])]});
 });
 const opponentId=String(fixture.home===USER_ID?fixture.away:fixture.home),opponent=teamById(opponentId),leader=teamById(leaderId)||state.standings?.[leaderId];
 state.lastRoundResults=roundResults;
 state.history.push({matchday:state.matchday+1,opponent:opponent?.name||'Avversario',opponentId,home:String(fixture.home)===USER_ID,gf:0,ga:0,displayGf:0,displayGa:0,winnerId:'',goals:[],opponentGoals:[],highlights:[],commentary:[],matchNotPlayed:true,fgciLeaderRested:true,fgciRestLeaderId:leaderId,fgciRestLeaderName:leader?.name||'La capolista',pointsAwarded:0,pointsAdjustment:0,pointsNote:'Regolamento FGCI: la capolista non disputa la partita.'});
 state.lastResult=state.history[state.history.length-1];postMatch();save();
 modalRoot.innerHTML=`<div class="modal-backdrop"><div class="modal result-modal-expanded"><div class="label">Nuovo regolamento FGCI</div><div class="match-result"><div class="team-name">${esc(state.teamName)} vs ${esc(opponent?.name||'Avversario')}</div><div class="score">—</div><div class="goal-line"><b>🛌 Primo a riposo</b><br>${esc(leader?.name||'La capolista')} era prima in classifica: questa partita non è stata disputata e nessuna delle due squadre riceve punti, vittorie o gol.</div><button id="closeResult" class="btn primary result-continue-top">Continua</button></div></div></div>`;
 document.getElementById('closeResult').onclick=()=>{modalRoot.innerHTML='';render()};
}

function playRound(mode='instant'){
 if(state.pendingEvent&&!state.pendingEvent.resolved)return;
 prepareBottomHelpRound();
 const fgciPreviewFixture=userFixture(),fgciRoundRestId=currentFgciLeaderRestId();
 if(fgciRoundRestId&&(String(fgciPreviewFixture.home)===fgciRoundRestId||String(fgciPreviewFixture.away)===fgciRoundRestId)){playFgciLeaderRestRound(mode,fgciPreviewFixture,fgciRoundRestId);return;}
 if(fgciPenaltyLotteryActive()){playFgciPenaltyLotteryRound(mode);return;}
 rememberFinalDayLeaderForAchievements();
 const fixture=userFixture(),error404Corrupted=error404CorruptionActive(),formulaOneInjuryWalkover=formulaOneInjuryWalkoverForMatch(fixture.home,fixture.away),baseMatchMinutes=fgciEffectiveTimeActive()?90:currentMatchDuration();let matchMinutes=pinkCardMatchDuration(baseMatchMinutes);const spaceJamTiming=spaceJamMatchTiming(matchMinutes),matchStartMinute=spaceJamTiming.startMinute,matchSimulationMinutes=spaceJamTiming.remainingMinutes,spaceJamScheduledDuration=matchMinutes,pinkCardMinute=!error404Corrupted&&state.seasonRules?.pinkCardEndsMatch?matchMinutes:0;
 const userTeam=teamById(USER_ID)||{id:USER_ID,name:state.teamName,colors:activeUserClub().colorClub};
 const opponent=teamById(fixture.home===USER_ID?fixture.away:fixture.home),bottomHalfOpponentRankBefore=bottomHalfUnbeatenOpponentRank(opponent?.id),questOpponentTop3Before=questOpponentIsTopThree(opponent?.id),lineup=resolveLineup(),fantaballopoliRule=fantaballopoliMatchRule(),opponentLineup=fantaballopoliOpponentLineup(teamMatchLineup(opponent),opponent,fantaballopoliRule),baseUserPower=(typeof sharedRegulationEffectivePower==='function'?sharedRegulationEffectivePower(USER_ID,matchPower()):matchPower()),userHome=fixture.home===USER_ID,hungerWalkover=!error404Corrupted&&isTeamEliminated(opponent?.id);
 const fantaballopoliRisks=formulaOneInjuryWalkover?{expelled:[],injured:[],powerPenalty:0}:applyFantaballopoliOpponentRisks(opponent,opponentLineup,fantaballopoliRule);
 const guaranteedRed=state.activeEffects.some(effect=>effect.type==='opponentRedCard'),chaos=state.activeEffects.filter(effect=>effect.type==='refChaos'),longRisk=Boolean(state.seasonRules.longMatchRisk&&matchMinutes===120),effectiveTimeRisk=fgciEffectiveTimeActive();
 const opponentRedChance=Math.min(1,(guaranteedRed?1:Math.max(0,...chaos.map(effect=>Number(effect.opponentRedChance)||0)))+(longRisk?0.15:0)+(effectiveTimeRisk?.52:0));
 const ownRedChance=Math.min(1,Math.max(0,...chaos.map(effect=>Number(effect.ownRedChance??effect.ownSuspensionChance)||0))+(longRisk?0.25:0)+(effectiveTimeRisk?.52:0));
 ...
 const rokkyOpponentBonus=coachIs('rokky')?5:0;
 const opponentPower=Math.max(1,opponentMatchPower(opponent)+opponentEventOvr+deathMatchBonus+rokkyOpponentBonus+(Number(fantaballopoliRule.opponentOvrBonus)||0)+(fantaballopoliRule.extraOpponentPlayer?8:0)-(redCandidate?10:0)-fantaballopoliRisks.powerPenalty),userPower=Math.max(35,baseUserPower-(ownRedEntry?10:0));
 ...
 const improvisedPenaltyOutcome=(!formulaOneInjuryWalkover&&!hungerWalkover&&!error404Corrupted)?applyImprovisedPenaltyDuringMatch(userGoalEvents,lineup,userTeam,opponent,matchMinutes):null;
 const figcPendingInjuries=prepareFigcNegativeUserInjuries(matchMinutes,Boolean(formulaOneInjuryWalkover));
 const goalkeeperScorerOutcome=(!formulaOneInjuryWalkover&&!hungerWalkover&&!error404Corrupted)?applyGoalkeeperScorerDuringMatch(userGoalEvents,opponentGoalEvents,lineup,opponentLineup,userTeam,opponent,matchMinutes):null;
 const weakBrotherGoalOutcome=(!formulaOneInjuryWalkover&&!hungerWalkover&&!error404Corrupted)?applyWeakBrotherOpponentGoal(opponentGoalEvents,opponent,userTeam,matchMinutes):null;
 const badContractOutcome=(!formulaOneInjuryWalkover&&!hungerWalkover&&!error404Corrupted)?applyBadContractGoalRule(userGoalEvents):null;
 const blessedBallOutcome=(!formulaOneInjuryWalkover&&!hungerWalkover&&!error404Corrupted&&!forcedScoreEffect&&typeof applyBlessedBallGoal==='function')?applyBlessedBallGoal(userGoalEvents,lineup,userTeam,opponent,matchMinutes):null;
 const fantaballopoliPenaltyApplied=applyFantaballopoliPenaltyGoal(opponentGoalEvents,opponent,userTeam,matchMinutes,fantaballopoliRule),fantaballopoliInjuredDoubleCount=applyFantaballopoliInjuredGoalValues(userGoalEvents,lineup,fantaballopoliRule);
 ...
 const figcUserIncidentOutcome=applyFigcNegativeGoalPenalty(goalsFor,userGoalEvents,{redCards:ownRedEntry?1:0,injuries:(figcPendingInjuries.eventInjuries?.length||0)+(figcPendingInjuries.longMatchInjury?1:0),missedPenalties:improvisedPenaltyOutcome?.awarded&&!improvisedPenaltyOutcome?.scored?1:0});
 const figcOpponentIncidentOutcome=applyFigcNegativeGoalPenalty(goalsAgainst,opponentGoalEvents,{redCards:redCandidate?1:0,injuries:fantaballopoliRisks.injured?.length||0,missedPenalties:0});
 goalsFor=figcUserIncidentOutcome.after;goalsAgainst=figcOpponentIncidentOutcome.after;
 prepareWweRiskInjuries(figcPendingInjuries,lineup);
 const wweWatchOutOutcome=applyWweWatchOutToMatch({teamA:userTeam,teamB:opponent,lineupA:lineup,lineupB:opponentLineup,eventsA:userGoalEvents,eventsB:opponentGoalEvents,redsA:ownRedEntry?[ownRedEntry]:[],redsB:redCandidate?[redCandidate]:[],injuriesA:[...(figcPendingInjuries.eventInjuries||[]),...(figcPendingInjuries.longMatchInjury?[figcPendingInjuries.longMatchInjury]:[])],injuriesB:fantaballopoliRisks.injured||[],duration:matchMinutes,userSideA:true,userSideB:false});
 if(wweWatchOutOutcome){goalsFor=wweWatchOutOutcome.scoreA;goalsAgainst=wweWatchOutOutcome.scoreB;}
 const extremeVarOutcome=(!formulaOneInjuryWalkover&&!error404Corrupted&&!hungerWalkover)?applyExtremeVarToGoalEvents(userGoalEvents,opponentGoalEvents):null;
 if(extremeVarOutcome){if(extremeVarOutcome.side==='user')goalsFor=Math.max(0,goalsFor-extremeVarOutcome.value);else goalsAgainst=Math.max(0,goalsAgainst-extremeVarOutcome.value);if(wweWatchOutOutcome){wweWatchOutOutcome.scoreA=goalsFor;wweWatchOutOutcome.scoreB=goalsAgainst;}}
 const sharedScoreOutcome=(!formulaOneInjuryWalkover&&!error404Corrupted&&!hungerWalkover&&typeof sharedRegulationScoreEffects==='function')?sharedRegulationScoreEffects({homeTeam:userTeam,awayTeam:opponent,homeEvents:userGoalEvents,awayEvents:opponentGoalEvents,homeScore:goalsFor,awayScore:goalsAgainst,homePower:userPower,awayPower:opponentPower,duration:matchMinutes}):{homeScore:goalsFor,awayScore:goalsAgainst,notes:[]};
 goalsFor=sharedScoreOutcome.homeScore;goalsAgainst=sharedScoreOutcome.awayScore;
 ...
 else{noDrawOutcome=error404Corrupted?error404Outcome(goalsFor,goalsAgainst,userTeam,opponent,matchMinutes):resolveNoDrawMatch({scoreA:goalsFor,scoreB:goalsAgainst,eventsA:userGoalEvents,eventsB:opponentGoalEvents,lineupA:lineup,lineupB:opponentLineup,teamA:userTeam,teamB:opponent,powerA:userPower,powerB:opponentPower,duration:matchMinutes,force:Boolean(fantaballopoliRule.forceNoDraw)});officialGoalsFor=noDrawOutcome.scoreA;officialGoalsAgainst=noDrawOutcome.scoreB;goalsFor=noDrawOutcome.outcomeScoreA;goalsAgainst=noDrawOutcome.outcomeScoreB;matchMinutes=noDrawOutcome.duration;}
 let buffonGlovesRemoved=0;if(!formulaOneWalkoverView&&!error404Corrupted&&typeof buffonGlovesActive==='function'&&buffonGlovesActive()&&Number(officialGoalsAgainst)>2){const before=Number(officialGoalsAgainst)||0;capGoalEvents(opponentGoalEvents,2);officialGoalsAgainst=2;goalsAgainst=Math.min(Number(goalsAgainst)||0,2);noDrawOutcome.scoreB=officialGoalsAgainst;noDrawOutcome.outcomeScoreB=goalsAgainst;if(officialGoalsFor>officialGoalsAgainst)noDrawOutcome.winnerId=String(USER_ID);else if(officialGoalsFor<officialGoalsAgainst)noDrawOutcome.winnerId=String(opponent?.id||'');else if(!noDrawOutcome.penalties)noDrawOutcome.winnerId='';buffonGlovesRemoved=Math.max(0,before-officialGoalsAgainst);}
 let bottomHalfUnbeatenTriggered=false;
 if(!fantaballopoliRule.forceNoDraw&&bottomHalfOpponentRankBefore>=11&&state.seasonRules?.bottomHalfUnbeaten&&Number(officialGoalsFor)<Number(officialGoalsAgainst)){
   const targetScore=Math.max(0,Number(officialGoalsAgainst)||0);while(scoreGoalEvents(userGoalEvents)<targetScore){const equalizer=regulationGoalEvent(userTeam,opponent,matchMinutes,'Clausola del presidente tirchio');equalizer.isBottomHalfProtectionGoal=true;equalizer.description=`La clausola del presidente impedisce la sconfitta contro la ${bottomHalfOpponentRankBefore}ª in classifica.`;userGoalEvents.push(equalizer)}userGoalEvents.sort((a,b)=>a.minute-b.minute);
   officialGoalsFor=targetScore;goalsFor=targetScore;goalsAgainst=targetScore;officialGoalsAgainst=targetScore;noDrawOutcome={...noDrawOutcome,scoreA:targetScore,scoreB:targetScore,outcomeScoreA:targetScore,outcomeScoreB:targetScore,winnerId:'',extraTime:false,penalties:null,note:`Clausola presidente tirchio: sconfitta trasformata in pareggio contro la ${bottomHalfOpponentRankBefore}ª.`};bottomHalfUnbeatenTriggered=true;
 }
 const soulDoubleOutcome=state.seasonRules?.soulDoubleMatchPending?(()=>{const second=simulateSoulSecondLeg({userHome,userPower,opponentPower,lineup,opponentLineup,userTeam,opponent,matchMinutes,matchStartMinute});return{first:{gf:Number(officialGoalsFor)||0,ga:Number(officialGoalsAgainst)||0,won:Number(officialGoalsFor)>Number(officialGoalsAgainst)},second,success:Number(officialGoalsFor)>Number(officialGoalsAgainst)&&second.won};})():null;
 ...
 let homeNoDrawOutcome={...noDrawOutcome,scoreA:homeGoals,scoreB:awayGoals,outcomeScoreA:userHome?noDrawOutcome.outcomeScoreA:noDrawOutcome.outcomeScoreB,outcomeScoreB:userHome?noDrawOutcome.outcomeScoreB:noDrawOutcome.outcomeScoreA,penalties:noDrawOutcome.penalties?(userHome?noDrawOutcome.penalties:{home:noDrawOutcome.penalties.away,away:noDrawOutcome.penalties.home,winnerId:noDrawOutcome.penalties.winnerId}):null};
 let commentary=formulaOneWalkoverView?[{minute:0,type:'fulltime',icon:'🚑',title:'Partita non disputata',text:formulaOneWalkoverView.doubleForfeit?'Entrambe le squadre hanno almeno un infortunato: doppia sconfitta 0-3 a tavolino.':formulaOneWalkoverView.userForfeit?'La tua squadra ha almeno un infortunato e perde 0-3 a tavolino.':'Gli avversari hanno almeno un infortunato: vittoria 3-0 a tavolino.',teamId:'',scoreAfter:`${homeGoals}–${awayGoals}`}]:buildMatchCommentary({homeTeam,awayTeam,homeLineup,awayLineup,homeEvents,awayEvents,homePower:userHome?userPower:opponentPower,awayPower:userHome?opponentPower:userPower,redCandidate,ownRedEntry,userHome,duration:matchMinutes,startMinute:matchStartMinute,pinkCard:Boolean(pinkCardMinute),opponentYellowRed,ownYellowRed,noDrawOutcome:homeNoDrawOutcome});
 if(sharedScoreOutcome?.notes?.length){sharedScoreOutcome.notes.forEach(text=>commentary.push({minute:matchMinutes,type:'rule',icon:'⚖️',title:'Regolamento federale',text,teamId:'',scoreAfter:`${homeGoals}–${awayGoals}`}));commentary.sort((a,b)=>Number(a.minute)-Number(b.minute)||(a.type==='fulltime'?1:b.type==='fulltime'?-1:0));}
 if(extremeVarOutcome){commentary.push({minute:extremeVarOutcome.minute||matchMinutes,type:'var',icon:'📺',title:`VAR estremamente severo · ${extremeVarOutcome.mode==='first'?'Troppo presto!':'Troppo tardi!'}`,text:`Il gol di ${extremeVarOutcome.player} è stato annullato.`,teamId:'',scoreAfter:`${homeGoals}–${awayGoals}`});commentary.sort((a,b)=>Number(a.minute)-Number(b.minute)||(a.type==='fulltime'?1:b.type==='fulltime'?-1:0))}
 if(bottomHalfUnbeatenTriggered){commentary.push({minute:matchMinutes,type:'var',icon:'💰',title:'Clausola del presidente tirchio',text:`${opponent?.name||'L’avversario'} era ${bottomHalfOpponentRankBefore}ª: la sconfitta viene trasformata in pareggio.`,teamId:'',scoreAfter:`${homeGoals}–${awayGoals}`});commentary.sort((a,b)=>Number(a.minute)-Number(b.minute)||(a.type==='fulltime'?1:b.type==='fulltime'?-1:0))}
 let committed=false,rokkyDecisionShown=false,rokkyOutcome=null;
 const commitRound=()=>{
   if(committed)return;committed=true;const roundResults=[],sharedPreRound=sharedRegulationPreRoundContext();let userAwards=null,userPoints={adjustment:0,awarded:0,note:''},formulaOneRound={ranking:[],user:null},figcBraceDetails=[];
   currentRound().forEach(match=>{
     const roundHomeTeam=teamById(match.home),roundAwayTeam=teamById(match.away),roundFormulaWalkover=match===fixture?formulaOneInjuryWalkover:formulaOneInjuryWalkoverForMatch(match.home,match.away);let homeScore,awayScore,roundHomeLineup,roundAwayLineup,roundHomeEvents,roundAwayEvents,roundWinnerId='',roundNoDrawOutcome=null,roundDurationUsed=90,roundStartMinute=0,roundHomeRed=false,roundAwayRed=false,roundFigcHomeIncident=null,roundFigcAwayIncident=null,roundWweWatchOutOutcome=null,roundSharedScoreOutcome=null;
     if(fgciRoundRestId&&(String(match.home)===fgciRoundRestId||String(match.away)===fgciRoundRestId)){roundResults.push({matchday:state.matchday+1,homeId:match.home,awayId:match.away,homeName:roundHomeTeam?.name||'',awayName:roundAwayTeam?.name||'',rested:true,restLeaderId:fgciRoundRestId});return;}
     if(match===fixture){homeScore=homeGoals;awayScore=awayGoals;roundHomeLineup=homeLineup;roundAwayLineup=awayLineup;roundHomeEvents=homeEvents;roundAwayEvents=awayEvents;roundWinnerId=String(noDrawOutcome.winnerId||'');roundNoDrawOutcome=homeNoDrawOutcome;roundDurationUsed=spaceJamScheduledDuration;roundStartMinute=matchStartMinute;roundWweWatchOutOutcome=wweWatchOutOutcome;roundSharedScoreOutcome=sharedScoreOutcome;}
     else{
       ...
       else{const roundDuration=pinkCardMatchDuration(90),roundTiming=spaceJamMatchTiming(roundDuration),roundSimulationDuration=roundTiming.remainingMinutes;roundDurationUsed=roundDuration;roundStartMinute=roundTiming.startMinute;const homeYellowRed=Boolean((state.seasonRules?.yellowEqualsRed&&Math.random()<.45)||(fgciEffectiveTimeActive()&&Math.random()<.52)),awayYellowRed=Boolean((state.seasonRules?.yellowEqualsRed&&Math.random()<.45)||(fgciEffectiveTimeActive()&&Math.random()<.52));roundHomeRed=homeYellowRed;roundAwayRed=awayYellowRed;let base=simulateScore(Math.max(35,opponentMatchPower(roundHomeTeam)-(homeYellowRed?10:0)),Math.max(35,opponentMatchPower(roundAwayTeam)-(awayYellowRed?10:0)),.18,roundSimulationDuration);if(chaosEnabled())base=applyChaosScoreRules(roundHomeTeam,roundAwayTeam,base[0],base[1]);roundHomeEvents=buildTeamGoals(base[0],roundHomeLineup,roundHomeTeam,roundAwayTeam,[],roundDuration,roundStartMinute);roundAwayEvents=buildTeamGoals(base[1],roundAwayLineup,roundAwayTeam,roundHomeTeam,[],roundDuration,roundStartMinute);if(homeYellowRed){const sentOff=pick(roundHomeLineup.filter(entry=>entry?.player));if(sentOff)opponentStatusOf(roundHomeTeam,sentOff.playerId).suspension=Math.max(2,opponentStatusOf(roundHomeTeam,sentOff.playerId).suspension)}if(awayYellowRed){const sentOff=pick(roundAwayLineup.filter(entry=>entry?.player));if(sentOff)opponentStatusOf(roundAwayTeam,sentOff.playerId).suspension=Math.max(2,opponentStatusOf(roundAwayTeam,sentOff.playerId).suspension)}if(fgciEffectiveTimeActive()){effectiveTimeOpponentInjury(roundHomeTeam,roundHomeLineup);effectiveTimeOpponentInjury(roundAwayTeam,roundAwayLineup)}if(state.seasonRules.redCardGoals){if(homeYellowRed||Math.random()<.07)roundHomeEvents.push(regulationGoalEvent(roundHomeTeam,roundAwayTeam,roundDuration));if(awayYellowRed||Math.random()<.07)roundAwayEvents.push(regulationGoalEvent(roundAwayTeam,roundHomeTeam,roundDuration));}normalizeGoalEventsToWindow(roundHomeEvents,roundStartMinute,roundDuration);normalizeGoalEventsToWindow(roundAwayEvents,roundStartMinute,roundDuration);roundHomeEvents.sort((a,b)=>a.minute-b.minute);roundAwayEvents.sort((a,b)=>a.minute-b.minute);const federationRound=applyFederationGoalRuleToEvents(roundHomeEvents,roundAwayEvents,roundHomeTeam,roundAwayTeam);homeScore=federationRound?federationRound.scoreA:scoreGoalEvents(roundHomeEvents);awayScore=federationRound?federationRound.scoreB:scoreGoalEvents(roundAwayEvents);}
     }
     ...
     if(match!==fixture&&!roundFormulaWalkover&&!error404Corrupted){roundFigcHomeIncident=applyFigcNegativeGoalPenalty(homeScore,roundHomeEvents,{redCards:roundHomeRed?1:0,injuries:0,missedPenalties:0});roundFigcAwayIncident=applyFigcNegativeGoalPenalty(awayScore,roundAwayEvents,{redCards:roundAwayRed?1:0,injuries:0,missedPenalties:0});homeScore=roundFigcHomeIncident.after;awayScore=roundFigcAwayIncident.after;roundWweWatchOutOutcome=applyWweWatchOutToMatch({teamA:roundHomeTeam,teamB:roundAwayTeam,lineupA:roundHomeLineup,lineupB:roundAwayLineup,eventsA:roundHomeEvents,eventsB:roundAwayEvents,redsA:roundHomeRed?[roundHomeLineup[0]]:[],redsB:roundAwayRed?[roundAwayLineup[0]]:[],injuriesA:[],injuriesB:[],duration:roundDurationUsed});if(roundWweWatchOutOutcome){homeScore=roundWweWatchOutOutcome.scoreA;awayScore=roundWweWatchOutOutcome.scoreB;}const sharedHomePower=opponentMatchPower(roundHomeTeam),sharedAwayPower=opponentMatchPower(roundAwayTeam);roundSharedScoreOutcome=typeof sharedRegulationScoreEffects==='function'?sharedRegulationScoreEffects({homeTeam:roundHomeTeam,awayTeam:roundAwayTeam,homeEvents:roundHomeEvents,awayEvents:roundAwayEvents,homeScore,awayScore,homePower:sharedHomePower,awayPower:sharedAwayPower,duration:roundDurationUsed}):{homeScore,awayScore,notes:[]};homeScore=roundSharedScoreOutcome.homeScore;awayScore=roundSharedScoreOutcome.awayScore;}
     ...
     const homeWasEliminated=isTeamEliminated(match.home),awayWasEliminated=isTeamEliminated(match.away),homeOutcome=matchOutcomeScores(homeScore,awayScore,roundWinnerId,match.home),awayOutcome=matchOutcomeScores(awayScore,homeScore,roundWinnerId,match.away),sharedPointsBefore={home:Number(state.standings?.[match.home]?.pts)||0,away:Number(state.standings?.[match.away]?.pts)||0};
     if(roundFormulaWalkover?.doubleForfeit){if(!homeWasEliminated)formulaOneApplyDoubleForfeitStanding(match.home);if(!awayWasEliminated)formulaOneApplyDoubleForfeitStanding(match.away);}else{if(!homeWasEliminated)updateStanding(match.home,homeScore,awayScore,{winnerId:roundWinnerId,away:false});if(!awayWasEliminated)updateStanding(match.away,awayScore,homeScore,{winnerId:roundWinnerId,away:true});}if(match===fixture)userPoints=applyUserPointRules(goalsFor,goalsAgainst);const sharedRegulationResult=(!roundFormulaWalkover&&!homeWasEliminated&&!awayWasEliminated)?applySharedRegulationPointsToMatch({homeId:match.home,awayId:match.away,homeScore,awayScore,winnerId:roundWinnerId,homeGoals:roundHomeEvents,awayGoals:roundAwayEvents,preRoundContext:sharedPreRound,beforePoints:sharedPointsBefore}):{homeAwarded:0,awayAwarded:0,homeAdjustment:0,awayAdjustment:0,notesByTeam:{},notes:[]};if(match===fixture){const userSide=userHome?'home':'away',userSharedNotes=sharedRegulationResult.notesByTeam?.[USER_ID]||[];userPoints.awarded=Number(sharedRegulationResult[`${userSide}Awarded`])||0;userPoints.adjustment=userPoints.awarded-baseLeaguePoints(goalsFor,goalsAgainst);if(userSharedNotes.length){userPoints.note=[userPoints.note,...userSharedNotes].filter(Boolean).join(' ');userSharedNotes.forEach(text=>commentary.push({minute:matchMinutes,type:'rule',icon:'⚖️',title:'Regolamento federale',text,teamId:USER_ID,scoreAfter:`${homeGoals}–${awayGoals}`}));commentary.sort((a,b)=>Number(a.minute)-Number(b.minute)||(a.type==='fulltime'?1:b.type==='fulltime'?-1:0));}}
     ...
     roundResults.push({matchday:state.matchday+1,homeId:match.home,awayId:match.away,homeName:roundHomeTeam?.name||'',awayName:roundAwayTeam?.name||'',homeScore,awayScore,winnerId:roundWinnerId,noDrawOutcome:roundNoDrawOutcome,spaceJamStartMinute:roundStartMinute,spaceJamScheduledDuration:roundDurationUsed,homeGoals:roundHomeEvents,awayGoals:roundAwayEvents,mvpId:awards.mvpId,mvpScore:awards.mvpScore,mvpTeamId:awards.mvpTeamId,formulaOneInjuryWalkover:roundFormulaWalkover?{...roundFormulaWalkover}:null,figcHomeIncident:match===fixture?(userHome?figcUserIncidentOutcome:figcOpponentIncidentOutcome):roundFigcHomeIncident,figcAwayIncident:match===fixture?(userHome?figcOpponentIncidentOutcome:figcUserIncidentOutcome):roundFigcAwayIncident,wweWatchOutOutcome:roundWweWatchOutOutcome?{...roundWweWatchOutOutcome}:null,sharedRegulationNotes:[...(roundSharedScoreOutcome?.notes||[]),...(sharedRegulationResult.notes||[])],eliminationNote});if(match===fixture){userAwards=awards;if(eliminationNote)userPoints.note=[userPoints.note,eliminationNote].filter(Boolean).join(' ')};
   });
   ...
   figcBraceDetails=applyFigcBraceBonusesToRound(roundResults);const userBraceDetail=figcBraceDetails.find(item=>String(item.teamId)===String(USER_ID));if(userBraceDetail){const bonus=Number(userBraceDetail.bonus)||0;userPoints.adjustment=(Number(userPoints.adjustment)||0)+bonus;userPoints.awarded=(Number(userPoints.awarded)||0)+bonus;userPoints.note=[userPoints.note,`Regola FIGC positiva: ${userBraceDetail.scorers.map(item=>item.playerName).join(', ')} ${bonus===1?'porta':'portano'} +${bonus} ${bonus===1?'punto':'punti'} grazie ${bonus===1?'alla doppietta':'alle doppiette'}.`].filter(Boolean).join(' ');}
   ...
   if(soulDoubleOutcome){const target=soulDoubleOutcome.success?6:0,standing=userStanding(),current=Number(userPoints.awarded)||0,delta=target-current;if(standing)standing.pts=(Number(standing.pts)||0)+delta;userPoints.adjustment=(Number(userPoints.adjustment)||0)+delta;userPoints.awarded=target;userPoints.note=`Patto dell’anima: prima partita ${soulDoubleOutcome.first.gf}-${soulDoubleOutcome.first.ga}, seconda partita ${soulDoubleOutcome.second.gf}-${soulDoubleOutcome.second.ga}. ${soulDoubleOutcome.success?'Hai vinto entrambe: +6 punti.':'Non hai vinto entrambe: 0 punti.'}`;state.seasonRules.soulDoubleMatchPending=false;}
   const effectiveTimeOpponentInjuryOutcome=!formulaOneWalkoverView?effectiveTimeOpponentInjury(opponent,opponentLineup):null;
   ...
   state.history.push({matchday:state.matchday+1,opponent:opponent.name,opponentId:opponent.id,home:userHome,gf:goalsFor,ga:goalsAgainst,displayGf:officialGoalsFor,displayGa:officialGoalsAgainst,winnerId:String(noDrawOutcome.winnerId||''),extraTime:Boolean(noDrawOutcome.extraTime),penalties:noDrawOutcome.penalties?{for:Number(noDrawOutcome.penalties.home)||0,against:Number(noDrawOutcome.penalties.away)||0,winnerId:String(noDrawOutcome.penalties.winnerId||'')}:null,noDrawRuleNote:String(noDrawOutcome.note||''),formulaOnePosition:Number(formulaOneRound.user?.position)||0,formulaOnePoints:Number(formulaOneRound.user?.points)||0,formulaOneRanking:formulaOneRound.ranking||[],goals:userGoalEvents,opponentGoals:opponentGoalEvents,highlights:allHighlights,commentary,power:Math.round(userPower*10)/10,opponentPower:Math.round(opponentPower*10)/10,rokkyOpponentBonus,rokkyOutcome:rokkyOutcome?{...rokkyOutcome}:null,extremeVarOutcome:extremeVarOutcome?{...extremeVarOutcome}:null,wweWatchOutOutcome:wweWatchOutOutcome?{...wweWatchOutOutcome}:null,soulDoubleOutcome:soulDoubleOutcome?{first:{...soulDoubleOutcome.first},second:{...soulDoubleOutcome.second},success:Boolean(soulDoubleOutcome.success)}:null,fgciDirectMatchRule:fgciDirectMatchRule(),effectiveTimeOpponentInjury:effectiveTimeOpponentInjuryOutcome?{...effectiveTimeOpponentInjuryOutcome}:null,opponentRedCard:Boolean(redCandidate),opponentRedPlayer:redCandidate?.name||'',ownRedCard:Boolean(ownRedEntry),ownSuspensionId:ownRedEntry?.playerId||'',ownSuspensionPlayer:ownRedEntry?.player?.name||'',goalkeeperId:userHome?userAwards?.homeGoalkeeperId:userAwards?.awayGoalkeeperId,opponentGoalkeeperId:userHome?userAwards?.awayGoalkeeperId:userAwards?.homeGoalkeeperId,cleanSheet:!formulaOneWalkoverView&&Number(officialGoalsAgainst)===0,opponentCleanSheet:!formulaOneWalkoverView&&Number(officialGoalsFor)===0,mvpId:userAwards?.mvpId||'',mvpScore:userAwards?.mvpScore||0,mvpTeamId:userAwards?.mvpTeamId||'',lineup:safeMatchLineup(lineup),opponentLineup:safeMatchLineup(opponentLineup),baseOvrOnlyActive:Boolean(state.activeEffects.some(effect=>effect.type==='baseOvrOnly')),forcedLoss,forcedWin,maxDraw,forcedScore:Boolean(forcedScoreEffect),forcedScoreData:forcedScoreEffect?{gf:Math.max(0,Number(forcedScoreEffect.gf)||0),ga:Math.max(0,Number(forcedScoreEffect.ga)||0),source:String(forcedScoreEffect.source||'')}:null,pointsAwarded:userPoints.awarded,pointsAdjustment:userPoints.adjustment,pointsNote:userPoints.note,forcedSubscriberGoals:forcedSubscribers.length,playMode:mode,matchDuration:matchMinutes,pinkCardMinute,opponentYellowRed,ownYellowRed,deathMatchBonus,hungerWalkover,futureScorerGoal:Boolean(futureGoal),meritStoryGoal:Boolean(meritStoryGoal),fantaballopoliForcedLoss:Boolean(fantaballopoliRule.forcedLoss),fantaballopoliForcedWin:Boolean(fantaballopoliRule.forcedWin),fantaballopoliNegativeGoals:Boolean(fantaballopoliRule.negativeOpponentGoals),fantaballopoliChallengeId:String(fantaballopoliRule.challengeId||''),fantaballopoliOpponentOvrBonus:Number(fantaballopoliRule.opponentOvrBonus)||0,fantaballopoliOpponentTwelve:Boolean(fantaballopoliRule.extraOpponentPlayer),fantaballopoliPenaltyApplied:Boolean(fantaballopoliPenaltyApplied),fantaballopoliOpeningGoalApplied:Boolean(fantaballopoliRule.openingGoalAgainst&&fantaballopoliPenaltyApplied),fantaballopoliInjuredDoubleCount:Number(fantaballopoliInjuredDoubleCount)||0,fantaballopoliExpulsions:fantaballopoliRisks.expelled.map(entry=>entry.player?.name||'Giocatore'),fantaballopoliInjuries:fantaballopoliRisks.injured.map(entry=>entry.player?.name||'Giocatore'),coachType:normalizeCoachType(state.coachType),coachOvrModifier:coachMatchOvrModifier(),coachGoalForGuaranteed:Boolean(coachGuarantees.for),coachGoalAgainstGuaranteed:Boolean(coachGuarantees.against),coachRosterAverage:Math.round(Number(coachGuarantees.average||coachRosterAverageOvr())*10)/10,ductilityBoosts:Array.isArray(userAwards?.ductilityBoosts)?userAwards.ductilityBoosts:[],curvaContestVenueMode:curvaContestVenueMode(),secretRefereePenalty,questOpponentTop3Before,bottomHalfOpponentRankBefore,bottomHalfUnbeatenTriggered,champagneDrawLoss,italiaCatenaccioRule:String(state.seasonRules?.italiaCatenaccioRule||''),italiaCatenaccioPenalty,allegriGoalsRemoved,fantaballaVideoRule:String(state.seasonRules?.fantaballaVideoRule||''),fantaballaVideoRuleNote:String(fantaballaVideoOutcome?.note||''),federationGoalRule:federationOutcome?.rule||'',federationGoalRuleNote:federationOutcome?.note||'',federationGoalWinnerId:federationOutcome?.winnerId||'',error404Corrupted:Boolean(error404Corrupted),formulaOneInjuryWalkover:formulaOneWalkoverView?{...formulaOneWalkoverView}:null,spaceJamRandomKickoff:Boolean(spaceJamTiming.active),spaceJamStartMinute:Number(matchStartMinute)||0,spaceJamScheduledDuration:Number(spaceJamScheduledDuration)||90,frenchLateBoosts:Array.isArray(frenchLateBoosts)?frenchLateBoosts:[],improvisedPenalty:improvisedPenaltyOutcome,figcIncidentRule:String(state.seasonRules?.figcIncidentRule||''),figcUserIncident:figcUserIncidentOutcome,figcOpponentIncident:figcOpponentIncidentOutcome,figcBraceBonus:Number(userBraceDetail?.bonus)||0,figcBraceScorers:Array.isArray(userBraceDetail?.scorers)?userBraceDetail.scorers:[],figcPendingInjuries:(figcNegativeIncidentRuleActive()||(figcPendingInjuries.eventInjuries||[]).length||figcPendingInjuries.longMatchInjury)?{eventInjuries:(figcPendingInjuries.eventInjuries||[]).map(item=>({...item})),longMatchInjury:figcPendingInjuries.longMatchInjury?{...figcPendingInjuries.longMatchInjury}:null}:null,goalkeeperScorer:goalkeeperScorerOutcome,weakBrotherGoal:weakBrotherGoalOutcome,badContract:badContractOutcome,buffonGlovesRemoved,blessedBall:blessedBallOutcome,noMisterMatch:typeof noMisterMatchState==='function'?{...noMisterMatchState()}:null});
 ...
 const applyRokkyGoalCancellation=()=>{
   let removedGoal=null;
   for(let index=opponentGoalEvents.length-1;index>=0;index--){const event=opponentGoalEvents[index];const value=Math.max(1,Number(event?.goalValue)||1);if(value>1){event.goalValue=value-1;removedGoal={...event,goalValue:1};}else{removedGoal=opponentGoalEvents.splice(index,1)[0]||null;}break;}
   officialGoalsAgainst=Math.max(0,Number(officialGoalsAgainst)||0)-1;goalsAgainst=officialGoalsAgainst;goalsFor=officialGoalsFor;
   const winnerId=officialGoalsFor>officialGoalsAgainst?String(USER_ID):officialGoalsFor<officialGoalsAgainst?String(opponent.id):'';
   noDrawOutcome={...noDrawOutcome,scoreA:officialGoalsFor,scoreB:officialGoalsAgainst,outcomeScoreA:officialGoalsFor,outcomeScoreB:officialGoalsAgainst,winnerId,extraTime:false,penalties:null,note:''};
   homeGoals=userHome?officialGoalsFor:officialGoalsAgainst;awayGoals=userHome?officialGoalsAgainst:officialGoalsFor;
   homeNoDrawOutcome={...noDrawOutcome,scoreA:homeGoals,scoreB:awayGoals,outcomeScoreA:userHome?officialGoalsFor:officialGoalsAgainst,outcomeScoreB:userHome?officialGoalsAgainst:officialGoalsFor};
   commentary=buildMatchCommentary({homeTeam,awayTeam,homeLineup,awayLineup,homeEvents,awayEvents,homePower:userHome?userPower:opponentPower,awayPower:userHome?opponentPower:userPower,redCandidate,ownRedEntry,userHome,duration:matchMinutes,startMinute:matchStartMinute,pinkCard:Boolean(pinkCardMinute),opponentYellowRed,ownYellowRed,noDrawOutcome:homeNoDrawOutcome});
 ...
 const finishRound=()=>{
   if(committed)return;
   const canUseRokky=coachIs('rokky')&&!(typeof noMisterCoachBonusesDisabled==='function'&&noMisterCoachBonusesDisabled())&&!fantaballopoliRule.forceNoDraw&&!rokkyDecisionShown&&!formulaOneWalkoverView&&!error404Corrupted&&!hungerWalkover&&Number(officialGoalsAgainst)>0;
   if(!canUseRokky){commitRound();return;}
   rokkyDecisionShown=true;
   const rokkyPreviewCommentary=Array.isArray(commentary)&&commentary.length?commentary.map(event=>commentaryRowHtml(event,homeTeam,awayTeam)).join(''):'<div class="goal-line">Partita senza azioni registrate.</div>';
   const rokkyPreviewScorers=`<div class="result-scorers">${renderResultScorers(userGoalEvents,state.teamName)}${renderResultScorers(opponentGoalEvents,opponent.name)}</div>`;
   modalRoot.innerHTML=`<div class="modal-backdrop"><div class="modal result-modal-expanded" role="dialog" aria-modal="true"><div class="label">Risultato finale · Rokky</div><div class="match-result"><div class="team-name">${esc(state.teamName)} vs ${esc(opponent.name)}</div><div class="score">${Number(officialGoalsFor)||0}–${Number(officialGoalsAgainst)||0}</div>${rokkyPreviewScorers}<div class="goal-line"><b>🕵️ Rokky può intervenire adesso</b><br>La partita è terminata. Puoi mantenere questo risultato oppure tentare di annullare un gol avversario.<br><b>80%</b> il gol viene annullato · <b>20%</b> vieni scoperto e perdi tutti i punti.</div><div class="top-actions"><button id="rokkyAttempt" class="btn red" type="button">Tenta l’annullamento</button><button id="rokkyKeepResult" class="btn gold" type="button">Mantieni il risultato</button></div><div class="match-highlights-title">Cronaca della partita</div><div class="goals match-highlights">${rokkyPreviewCommentary}</div></div></div></div>`;
   const lockRokkyChoice=()=>{['rokkyAttempt','rokkyKeepResult'].forEach(id=>{const button=document.getElementById(id);if(button)button.disabled=true})};
   document.getElementById('rokkyKeepResult').onclick=()=>{lockRokkyChoice();rokkyOutcome={attempted:false,success:false,caught:false};commitRound()};
   document.getElementById('rokkyAttempt').onclick=()=>{lockRokkyChoice();if(Math.random()<.8)applyRokkyGoalCancellation();else rokkyOutcome={attempted:true,success:false,caught:true,originalGoalsAgainst:Number(officialGoalsAgainst)||0,finalGoalsAgainst:Number(officialGoalsAgainst)||0};commitRound()};
 };
...
function postMatch(){
 if(state.lastResult?.fgciLeaderRested){Object.values(state.statuses).forEach(status=>{if(status.seasonOut)status.injury=Math.max(1,Number(status.injury)||1);else if(status.injury>0)status.injury--;if(status.suspension>0)status.suspension--;});state.teams.filter(team=>team.id!==USER_ID).forEach(team=>{Object.values(team.statuses||{}).forEach(status=>{if(status.injury>0)status.injury--;if(status.suspension>0)status.suspension--;});});state.activeEffects.forEach(effect=>{if(!effectLastsUntilSeasonEnd(effect))effect.rounds--});state.activeEffects=state.activeEffects.filter(effect=>effectLastsUntilSeasonEnd(effect)||effect.rounds>0);tickSharedRegulationsAfterRound();state.playInjured={};advanceSeasonAfterCompletedMatchday();return;}
 ...
 const result=state.lastResult,matchWasFormulaOneWalkover=Boolean(result?.formulaOneInjuryWalkover?.active),matchWasPenaltyLottery=Boolean(result?.penaltyLottery);if(!matchWasFormulaOneWalkover&&!matchWasPenaltyLottery&&!parallelCupDisciplineImmunity()&&result?.ownSuspensionId)statusOf(result.ownSuspensionId).suspension=Math.max(1,statusOf(result.ownSuspensionId).suspension);
 if(!matchWasFormulaOneWalkover&&!matchWasPenaltyLottery&&!parallelCupDisciplineImmunity()&&Math.random()<(cardEffect?.18:.07)){const entry=pick(rosterPlayers());if(entry)statusOf(entry.playerId).suspension=Math.max(1,statusOf(entry.playerId).suspension);}
 ...
 const figcInjuriesApplied=matchWasPenaltyLottery?false:applyPendingFigcUserInjuries(result),risks=state.activeEffects.filter(effect=>effect.type==='injuryRisk');
 if(!figcInjuriesApplied&&!matchWasFormulaOneWalkover&&!matchWasPenaltyLottery&&risks.length){const chance=physioAdjustedInjuryChance(1-risks.reduce((remaining,effect)=>remaining*(1-clamp(Number(effect.chance)||0,0,1)),1));if(Math.random()<chance){const count=Math.max(1,...risks.map(effect=>Number(effect.count)||1)),duration=Math.max(1,...risks.map(effect=>Number(effect.duration)||1)),candidates=shuffle(rosterPlayers()).slice(0,count);candidates.forEach(entry=>setOwnPlayerInjury(entry,duration));if(result)result.eventInjuries=candidates.map(entry=>({playerId:String(entry.playerId),name:entry.player.name,duration}));}}
 if(!figcInjuriesApplied&&!matchWasFormulaOneWalkover&&!matchWasPenaltyLottery&&state.seasonRules.longMatchRisk&&Number(result?.matchDuration)===120&&Math.random()<physioAdjustedInjuryChance(.55)){const entry=randomOwnEntry();if(entry){setOwnPlayerInjury(entry,2);result.longMatchInjury={playerId:String(entry.playerId),name:entry.player.name,duration:2};}}
 ...
 if(typeof resolveCaptainArmbandAfterMatch==='function')resolveCaptainArmbandAfterMatch(result);
 if(typeof resolveBlessedBallAfterMatch==='function')resolveBlessedBallAfterMatch(result);
 tickCreatorEventsAfterMatch(result);
 tickAdditionalUserEventsAfterMatch(result);
 ...
 state.activeEffects.forEach(effect=>{if(!effectLastsUntilSeasonEnd(effect))effect.rounds--});state.activeEffects=state.activeEffects.filter(effect=>effectLastsUntilSeasonEnd(effect)||effect.rounds>0);tickSharedRegulationsAfterRound();activateRestRewards(classicRestRewards,result);activatePendingNationalBoost();state.playInjured={};tickChaosOpponentEffects();resolvePendingParityResetAfterRound(result);resolveCurvaContestAfterRound(result);
 if(result?.rokkyOutcome?.caught){const standing=userStanding();const lostPoints=Math.max(0,Number(standing?.pts)||0);if(standing)standing.pts=0;result.rokkyOutcome.lostPoints=lostPoints;result.pointsAdjustment=(Number(result.pointsAdjustment)||0)-lostPoints;result.pointsAwarded=(Number(result.pointsAwarded)||0)-lostPoints;result.pointsNote=[result.pointsNote,`Rokky è stato scoperto: persi tutti i ${lostPoints} punti in classifica.`].filter(Boolean).join(' ');}
 applyHeroAcademiaFourOneAfterMatch(result);
 if(typeof applyCellPerfectAfterMatch==='function')applyCellPerfectAfterMatch(result);
 const cyborgRestored=typeof cyborgRestoreProtectedRoster==='function'?cyborgRestoreProtectedRoster():[];
 if(cyborgRestored.length){result.eventUpdates=Array.isArray(result.eventUpdates)?result.eventUpdates:[];result.eventUpdates.push({success:true,title:'Cyborg',message:`Cyborg neutralizza i malus e ripristina l’OVR di ${cyborgRestored.length} ${cyborgRestored.length===1?'giocatore':'giocatori'}.`})}
 state.matchday++;if(typeof trackLastPlaceFromMatchday10==='function')trackLastPlaceFromMatchday10();state.pendingEvent=null;
 if(state.matchday===19){runChaosOpponentMidseason();if(coachIs('three-five-two')||playerAcquisitionBlocked()){state.midseason={...state.midseason,step:0,target:0,outgoingId:'',mandatoryOutgoingId:'',mandatoryOutgoingIds:[],clubId:'',nation:'',candidates:[],pendingCandidateId:'',drawsUsed:0,completed:true,auto:false,autoCompleted:true,changes:[]};resolveFantaballopoliMidseason();if(isTeamEliminated(USER_ID)||state.matchday>=seasonLength()){advanceAfterRegularSeason()}else if(!curvaContestState().pendingTeamId)prepareEvent();}else{const target=midseasonTarget(),auto=Boolean(state.seasonRules.autoMidseason||state.seasonRules.botMidseason),mandatoryIds=mandatoryMidseasonPlayerIds().filter(id=>rosterEntry(id)).slice(0,3),mandatory=mandatoryIds[0]||'';state.midseason={step:0,target:clamp(Math.max(target,mandatoryIds.length),1,3),outgoingId:mandatory,mandatoryOutgoingId:mandatory,mandatoryOutgoingIds:mandatoryIds,clubId:'',nation:'',candidates:[],pendingCandidateId:'',drawsUsed:0,completed:false,auto,autoCompleted:false,changes:[]};state.phase='midseason';}}
...
function renderFinalScoreHero(result){
 const userTeam=typeof activeUserClub==='function'?activeUserClub():(teamById(USER_ID)||{name:state.teamName,colors:state.teamColors});
 const opponentTeam=teamById(result.opponentId)||(state.teams||[]).find(team=>String(team.name||'')===String(result.opponent||''))||{name:result.opponent||'Avversario'};
 const userPal=teamPalette(userTeam),opponentPal=teamPalette(opponentTeam);
 const userScore=Number.isFinite(Number(result.displayGf))?Number(result.displayGf):Number(result.gf)||0;
 const opponentScore=Number.isFinite(Number(result.displayGa))?Number(result.displayGa):Number(result.ga)||0;
 const badge=name=>String(name||'?').trim().split(/\s+/).map(part=>part[0]||'').join('').slice(0,3).toUpperCase();
 const teamCard=(team,pal,side)=>{const teamName=String(team.name||'Squadra').trim(),nameLength=teamName.length,nameSize=nameLength>32?16:nameLength>25?18:nameLength>19?21:nameLength>14?25:30;return `<div class="result-hero-team result-hero-team-${side}" style="--result-primary:${pal.primary};--result-secondary:${pal.secondary};--result-accent:${pal.accent};--result-text:${pal.text};--result-name-size:${nameSize}px"><span class="result-hero-badge" aria-hidden="true"></span><strong>${esc(teamName)}</strong></div>`};
 return `<div class="result-score-hero">${teamCard(userTeam,userPal,'home')}<div class="result-score-center"><div class="result-score-main"><span>${userScore}</span><i>–</i><span>${opponentScore}</span></div>${result.penalties?`<small>${Number(result.penalties.for)||0}–${Number(result.penalties.against)||0} d.c.r.</small>`:''}</div>${teamCard(opponentTeam,opponentPal,'away')}</div>`;
}
...
function showResultModal(){
 ...
 const suspensionCancelled=result.varTokenUsed?.type==='red'&&String(result.varTokenUsed?.playerId||'')===String(result.ownSuspensionId||'');
 const suspensionLine=result.ownSuspensionPlayer&&!suspensionCancelled?`<div class="goal-line red-card-event"><b>🟥 ${result.ownYellowRed?'Giallo trasformato in rosso':'Espulsione'}: ${esc(result.ownSuspensionPlayer)}</b><br>Sarà squalificato per la prossima giornata.</div>`:'';
 ...
 const fantaballopoliTrialLine=result.fantaballopoliChallengeId?`<div class="goal-line"><b>🕴️ Prova di Fantaballopoli</b><br>${result.fantaballopoliOpponentTwelve?'Gli avversari hanno giocato in 12. ':''}${Number(result.fantaballopoliOpponentOvrBonus)>0?`Avversari: +${Number(result.fantaballopoliOpponentOvrBonus)} OVR. `:''}${result.fantaballopoliOpeningGoalApplied?'La partita è iniziata dallo 0-1. ':result.fantaballopoliPenaltyApplied?'Rigore automatico assegnato contro di te. ':''}${Number(result.fantaballopoliInjuredDoubleCount)>0?`${Number(result.fantaballopoliInjuredDoubleCount)} ${Number(result.fantaballopoliInjuredDoubleCount)===1?'gol di un infortunato è valso doppio':'gol di giocatori infortunati sono valsi doppi'}. `:''}</div>`:'';
 const fantaballopoliChallengeLine=result.fantaballopoliChallengeOutcome?`<div class="goal-line ${result.fantaballopoliChallengeOutcome.success?'':'red-card-event'}"><b>${result.fantaballopoliChallengeOutcome.success?'✅':'❌'} ${esc(result.fantaballopoliChallengeOutcome.title||'Prova di Fantaballopoli')}</b><br>${esc(result.fantaballopoliChallengeOutcome.text||'')}</div>`:'';
 ...
 const extremeVarLine=result.extremeVarOutcome?`<div class="goal-line"><b>📺 VAR estremamente severo · ${result.extremeVarOutcome.mode==='first'?'Troppo presto!':'Troppo tardi!'}</b><br>Il gol di ${esc(result.extremeVarOutcome.player||'un giocatore')} al ${Number(result.extremeVarOutcome.minute)||0}′ è stato annullato, indipendentemente dalla squadra che lo aveva segnato.</div>`:'';
 const soulDoubleLine=result.soulDoubleOutcome?`<div class="goal-line ${result.soulDoubleOutcome.success?'':'red-card-event'}"><b>😈 Patto dell’anima · doppia partita</b><br>Prima simulazione: ${Number(result.soulDoubleOutcome.first?.gf)||0}-${Number(result.soulDoubleOutcome.first?.ga)||0}. Seconda simulazione: ${Number(result.soulDoubleOutcome.second?.gf)||0}-${Number(result.soulDoubleOutcome.second?.ga)||0}. ${result.soulDoubleOutcome.success?'Due vittorie: +6 punti.':'Non hai vinto entrambe: 0 punti.'}</div>`:'';
 const heroAcademiaLine=result.heroAcademiaOutcome?`<div class="goal-line ${result.heroAcademiaOutcome.type==='win-reset'?'red-card-event':''}"><b>🦸 All 4 1 & 1 4 all</b><br>${esc(result.heroAcademiaOutcome.message||'La regola del 4-1 ha modificato la classifica.')}</div>`:'';
 const penaltyLotteryLine=result.penaltyLottery?`<div class="goal-line"><b>🎯 Lotteria dei rigori</b><br>Partita iniziata direttamente dal dischetto: 0-0 regolamentare, ${Number(result.penalties?.for)||0}-${Number(result.penalties?.against)||0} ai rigori. Nessun infortunio e nessun rosso generato dalla partita.</div>`:'';
 const wweWatchOutLine=result.wweWatchOutOutcome?`<div class="goal-line"><b>🤼 Watch out!</b><br>I gol normali sono stati annullati. ${Number(result.wweWatchOutOutcome.scoreA)||0} gol per ${esc(state.teamName)} da infortuni/espulsioni e ${Number(result.wweWatchOutOutcome.scoreB)||0} per gli avversari.</div>`:'';
 const effectiveTimeLine=result.fgciDirectMatchRule==='effective-time'?`<div class="goal-line ${result.effectiveTimeOpponentInjury?'red-card-event':''}"><b>⏱️ Tempo effettivo</b><br>Partita disputata su 90 minuti effettivi con rischio drasticamente aumentato di infortuni ed espulsioni.${result.effectiveTimeOpponentInjury?` ${esc(result.effectiveTimeOpponentInjury.playerName)} (${esc(result.effectiveTimeOpponentInjury.teamName)}) si infortuna per ${Number(result.effectiveTimeOpponentInjury.duration)||1} giornate.`:''}</div>`:'';
 const pointsLine=result.pointsNote?`<div class="goal-line"><b>🏆 Punti campionato</b><br>${esc(result.pointsNote)}</div>`:'';
 const figcIncidentLine=result.figcIncidentRule==='negative'&&((Number(result.figcUserIncident?.penalty)||0)||(Number(result.figcOpponentIncident?.penalty)||0))?`<div class="goal-line red-card-event"><b>📋 Regola FIGC · Penalità episodi</b><br>${Number(result.figcUserIncident?.penalty)>0?`${esc(state.teamName)} perde ${Number(result.figcUserIncident.penalty)} ${Number(result.figcUserIncident.penalty)===1?'gol':'gol'} per ${[Number(result.figcUserIncident?.incidents?.redCards)>0?`${Number(result.figcUserIncident.incidents.redCards)} rossi`:'' ,Number(result.figcUserIncident?.incidents?.injuries)>0?`${Number(result.figcUserIncident.incidents.injuries)} infortuni`:'' ,Number(result.figcUserIncident?.incidents?.missedPenalties)>0?`${Number(result.figcUserIncident.incidents.missedPenalties)} rigori sbagliati`:'' ].filter(Boolean).join(', ')}. `:''}${Number(result.figcOpponentIncident?.penalty)>0?`${esc(result.opponent)} perde ${Number(result.figcOpponentIncident.penalty)} ${Number(result.figcOpponentIncident.penalty)===1?'gol':'gol'}.`:''}</div>`:'';
 const figcBraceLine=Number(result.figcBraceBonus)>0?`<div class="goal-line"><b>⚽ Regola FIGC · Premio doppietta</b><br>${(result.figcBraceScorers||[]).map(item=>esc(item.playerName)).join(', ')} ${Number(result.figcBraceBonus)===1?'porta':'portano'} +${Number(result.figcBraceBonus)} ${Number(result.figcBraceBonus)===1?'punto':'punti'} in classifica.</div>`:'';
 const noDrawLine=result.noDrawRuleNote&&!result.penaltyLottery?`<div class="goal-line"><b>🎯 Niente pareggio</b><br>${esc(result.noDrawRuleNote)}</div>`:'';
 ...
 const buffonGlovesLine=Number(result.buffonGlovesRemoved)>0?`<div class="goal-line"><b>🧤 Guanti di Buffon</b><br>La protezione permanente ha eliminato ${Number(result.buffonGlovesRemoved)} ${Number(result.buffonGlovesRemoved)===1?'gol subito':'gol subiti'} oltre il limite massimo di 2.</div>`:'';
 ...
 const visibleEventInjuries=(Array.isArray(result.eventInjuries)?result.eventInjuries:[]).filter(item=>!(result.varTokenUsed?.type==='injury'&&String(result.varTokenUsed?.playerId||'')===String(item.playerId||'')));
 const injuryLine=visibleEventInjuries.length?`<div class="goal-line"><b>🚑 Conseguenza dell’imprevisto</b><br>${visibleEventInjuries.map(item=>`${esc(item.name)}: ${item.duration} ${item.duration===1?'giornata':'giornate'}`).join(' · ')}</div>`:'';
 ...
 const itemLine=(Array.isArray(result.itemUpdates)?result.itemUpdates:[]).map(update=>`<div class="goal-line match-item-update ${update.success?'':'red-card-event'}"><b>${update.success?'🎒':'🗑️'} Oggetto: ${esc(update.title||'Oggetto')}</b><br>${esc(update.message||'')}</div>`).join('');
 const varTokenActionLine=typeof renderVarTokenResultActions==='function'?renderVarTokenResultActions(result):'';
 ...
 const rokkyLine=result.rokkyOutcome?.success?`<div class="goal-line"><b>🕵️ Rokky · gol annullato</b><br>Il tentativo è riuscito: un gol avversario è stato cancellato dal risultato finale.</div>`:result.rokkyOutcome?.caught?`<div class="goal-line red-card-event"><b>🚨 Rokky è stato scoperto</b><br>Il gol resta valido e hai perso tutti i ${Number(result.rokkyOutcome.lostPoints)||0} punti che avevi in classifica.</div>`:'';
 const rokkyOpponentLine=Number(result.rokkyOpponentBonus)>0?`<div class="goal-line red-card-event"><b>😡 Tutti ti odiano</b><br>L’avversario ha ricevuto +${Number(result.rokkyOpponentBonus)} OVR contro la tua squadra.</div>`:'';
 ...
 modalRoot.innerHTML=`<div class="modal-backdrop"><div class="modal result-modal-expanded"><div class="label">Risultato finale</div><div class="match-result">${renderFinalScoreHero(result)}<button id="closeResult" class="btn primary result-continue-top">Continua</button>${scorersLine}${pinkLine}${redLine}${suspensionLine}${forcedLine}${fantaballopoliMatchLine}${fantaballopoliTrialLine}${fantaballopoliChallengeLine}${fantaballopoliDisciplineLine}${error404Line}${spaceJamStartLine}${spaceJamOutcomeLine}${frenchLateBoostLine}${federationLine}${extremeVarLine}${soulDoubleLine}${heroAcademiaLine}${penaltyLotteryLine}${effectiveTimeLine}${wweWatchOutLine}${figcIncidentLine}${pointsLine}${figcBraceLine}${noDrawLine}${formulaOneWalkoverLine}${formulaOneLine}${buffonGlovesLine}${italiaCatenaccioLine}${parityResetLine}${curvaContestLine}${curvaVenueLine}${injuryLine}${ductilityLine}${secretRefereeLine}${classicEventLine}${futurePenaltyLine}${chickenTrainingLine}${chickenGoalLine}${penguinLine}${storyLine}${questLine}${itemLine}${varTokenActionLine}${cupLine}${champagneLine}${hungerLine}${subscriberLine}${creatorBoostLine}${coachResultLine}${rokkyOpponentLine}${rokkyLine}<div class="match-highlights-title">Cronaca della partita</div><div class="goals match-highlights">${commentaryHtml}</div>${mvpLine}</div></div></div>`;
 if(typeof bindVarTokenResultControls==='function')bindVarTokenResultControls();

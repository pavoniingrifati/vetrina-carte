...
function playLiveMatch({commentary,homeTeam,awayTeam,homeGoals,awayGoals,matchday,duration=90,startMinute=0,label=''},finishRound){
 ...
 modalRoot.innerHTML=`<div class="modal-backdrop"><section class="modal live-match-modal" role="dialog" aria-modal="true" aria-labelledby="liveMatchTitle"><header class="live-match-head"><div class="live-match-headline"><div><div class="label">${esc(label||`Giornata ${matchday}`)}</div><b id="liveMatchTitle">Diretta della partita</b><div class="live-match-subtitle">Segui la cronaca minuto per minuto oppure salta subito al risultato finale.</div></div><span class="live-status" aria-label="Partita in diretta">Live</span></div></header><div class="live-scoreboard" role="status" aria-live="polite" aria-atomic="true"><div class="live-team" style="${teamCssVars(homeTeam)}"><span class="live-team-colors"></span><b title="${esc(homeTeam.name)}">${esc(homeTeam.name)}</b><small>Casa</small></div><div class="live-score"><small>Risultato live</small><div><span id="liveHomeScore">0</span><em>–</em><span id="liveAwayScore">0</span></div></div><div class="live-team" style="${teamCssVars(awayTeam)}"><span class="live-team-colors"></span><b title="${esc(awayTeam.name)}">${esc(awayTeam.name)}</b><small>Trasferta</small></div></div><div class="live-clock-wrap"><span id="liveClock" class="live-clock">0'</span><div class="live-progress"><i id="liveProgress"></i></div></div><div id="liveCommentary" class="live-commentary"><div class="live-empty">La partita sta per iniziare…</div></div><footer class="live-match-controls"><div class="live-controls-tip">Puoi mettere in pausa oppure saltare direttamente al finale.</div><button id="pauseLiveMatch" class="btn secondary-strong" type="button">⏸ Pausa</button><button id="skipLiveMatch" class="btn match-instant-button" type="button" data-single-action data-busy-announcement="Simulazione del resto della partita avviata.">⚡ Vai al risultato finale</button></footer></section></div>`;
 ...
 const complete=()=>{if(closed)return;closed=true;if(timer)clearTimeout(timer);homeScoreNode.textContent=String(homeGoals);awayScoreNode.textContent=String(awayGoals);updateClock(totalMinutes);pauseButton.disabled=true;setTimeout(finishRound,1200)};
...
function showResultModal(){
 ...
 const verdict=result.gf>result.ga?'✅ Vittoria':result.gf<result.ga?'❌ Sconfitta':'➖ Pareggio';
 const modeLabel=result.playMode==='live'?'Cronaca live':'Simulazione rapida';
 const userDisplayGoals=Number.isFinite(Number(result.displayGf))?Number(result.displayGf):Number(result.gf)||0;
 const opponentDisplayGoals=Number.isFinite(Number(result.displayGa))?Number(result.displayGa):Number(result.ga)||0;
 const homeDisplayGoals=result.home?userDisplayGoals:opponentDisplayGoals;
 const awayDisplayGoals=result.home?opponentDisplayGoals:userDisplayGoals;
 const notesHtml=[pinkLine,redLine,suspensionLine,forcedLine,fantaballopoliMatchLine,fantaballopoliDisciplineLine,error404Line,spaceJamStartLine,spaceJamOutcomeLine,frenchLateBoostLine,federationLine,pointsLine,noDrawLine,formulaOneWalkoverLine,formulaOneLine,italiaCatenaccioLine,parityResetLine,curvaContestLine,curvaVenueLine,injuryLine,ductilityLine,secretRefereeLine,classicEventLine,futurePenaltyLine,chickenTrainingLine,chickenGoalLine,penguinLine,storyLine,questLine,cupLine,champagneLine,hungerLine,subscriberLine,creatorBoostLine,coachResultLine].filter(Boolean).join('');
 modalRoot.innerHTML=`<div class="modal-backdrop"><div class="modal result-modal-expanded"><div class="result-hero"><div class="result-hero-top"><div><div class="result-stage-badge">Giornata ${Number(result.matchday)||Number(state.matchday)||0}</div><div class="label" style="margin-top:8px;color:#ffe96c">Risultato finale</div></div><div class="result-verdict">${verdict}</div></div><div class="result-scoreboard"><div class="result-team-card" style="${teamCssVars(homeTeam)}"><span class="result-team-pip"></span><b title="${esc(homeTeam.name)}">${esc(homeTeam.name)}</b><small>${result.home?'La tua squadra':'Avversario'}</small></div><div class="result-big-score"><span>Finale</span><strong>${homeDisplayGoals}–${awayDisplayGoals}${result.penalties?` <small>(${Number(result.penalties.for)||0}–${Number(result.penalties.against)||0} d.c.r.)</small>`:''}</strong></div><div class="result-team-card" style="${teamCssVars(awayTeam)}"><span class="result-team-pip"></span><b title="${esc(awayTeam.name)}">${esc(awayTeam.name)}</b><small>${result.home?'Avversario':'La tua squadra'}</small></div></div><div class="result-summary-strip"><div class="result-chip"><span>Modalità</span><b>${esc(modeLabel)}</b></div><div class="result-chip"><span>Durata</span><b>${Number(result.matchDuration)||90} min</b></div><div class="result-chip"><span>Potenza tua</span><b>${Number(result.power)||0}</b></div><div class="result-chip"><span>Potenza avv.</span><b>${Number(result.opponentPower)||0}</b></div></div><button id="closeResult" class="btn primary result-continue-top">Continua stagione</button></div><div class="result-grid-two"><div class="result-side-stack"><section class="result-section"><h3>Marcatori</h3>${scorersLine}</section><section class="result-section"><div class="match-highlights-title">Cronaca della partita</div><div class="goals match-highlights">${commentaryHtml}</div></section></div><div class="result-side-stack">${mvpLine?`<section class="result-section"><h3>MVP</h3>${mvpLine}</section>`:''}${notesHtml?`<section class="result-section"><h3>Conseguenze e note</h3><div class="result-notes">${notesHtml}</div></section>`:''}</div></div></div></div>`;

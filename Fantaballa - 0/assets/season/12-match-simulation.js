...
function playRound(mode='instant'){
 ...
 if(extremeVarOutcome){if(extremeVarOutcome.side==='user')goalsFor=Math.max(0,goalsFor-extremeVarOutcome.value);else goalsAgainst=Math.max(0,goalsAgainst-extremeVarOutcome.value);}
 ...
 const commitRound=()=>{
   ...
   currentRound().forEach(match=>{
     ...
     roundResults.push({matchday:state.matchday+1,homeId:match.home,awayId:match.away,homeName:roundHomeTeam?.name||'',awayName:roundAwayTeam?.name||'',homeScore,awayScore,winnerId:roundWinnerId,noDrawOutcome:roundNoDrawOutcome,spaceJamStartMinute:roundStartMinute,spaceJamScheduledDuration:roundDurationUsed,homeGoals:roundHomeEvents,awayGoals:roundAwayEvents,mvpId:awards.mvpId,mvpScore:awards.mvpScore,mvpTeamId:awards.mvpTeamId,formulaOneInjuryWalkover:roundFormulaWalkover?{...roundFormulaWalkover}:null,figcHomeIncident:match===fixture?(userHome?figcUserIncidentOutcome:figcOpponentIncidentOutcome):roundFigcHomeIncident,figcAwayIncident:match===fixture?(userHome?figcOpponentIncidentOutcome:figcUserIncidentOutcome):roundFigcAwayIncident,eliminationNote});if(match===fixture){userAwards=awards;if(eliminationNote)userPoints.note=[userPoints.note,eliminationNote].filter(Boolean).join(' ')};
   });

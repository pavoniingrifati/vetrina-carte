...
function previewTeamAverageOvr(team){
 if(!team)return 0;
 if(String(team.id||'')===String(USER_ID)){const base=resolvedLineupAverage(resolveLineup())||0,effective=typeof sharedRegulationEffectivePower==='function'?sharedRegulationEffectivePower(USER_ID,base):base;return Math.round(effective)}
...
function playParallelCupMatch(mode='instant'){
 ...
 let [homeGoals,awayGoals]=simulateScore(homePower,awayPower,.18,duration),homeEvents=buildTeamGoals(homeGoals,homeLineup,homeTeam,awayTeam,[],duration),awayEvents=buildTeamGoals(awayGoals,awayLineup,awayTeam,homeTeam,[],duration);homeGoals=scoreGoalEvents(homeEvents);awayGoals=scoreGoalEvents(awayEvents);const sharedCupOutcome=typeof sharedRegulationScoreEffects==='function'?sharedRegulationScoreEffects({homeTeam,awayTeam,homeEvents,awayEvents,homeScore:homeGoals,awayScore:awayGoals,homePower,awayPower,duration}):{homeScore:homeGoals,awayScore:awayGoals,notes:[]};homeGoals=sharedCupOutcome.homeScore;awayGoals=sharedCupOutcome.awayScore;
 const commentary=buildMatchCommentary({homeTeam,awayTeam,homeLineup,awayLineup,homeEvents,awayEvents,homePower,awayPower,redCandidate,ownRedEntry,userHome,duration,pinkCard:Boolean(pinkCardMinute),opponentYellowRed,ownYellowRed});if(sharedCupOutcome.notes?.length){sharedCupOutcome.notes.forEach(text=>commentary.push({minute:duration,type:'rule',icon:'⚖️',title:'Regolamento federale',text,teamId:'',scoreAfter:`${homeGoals}–${awayGoals}`}));commentary.sort((a,b)=>Number(a.minute)-Number(b.minute)||(a.type==='fulltime'?1:b.type==='fulltime'?-1:0));}
 const finishCup=()=>{
...
function renderTable(){const rows=sortedTable(),ucl=isChampionsCompetition(),zones=typeof renderChampionsLeagueQualification==='function'?renderChampionsLeagueQualification():'';return `${ucl?zones:''}<div class="season-table-scroll" role="region" aria-label="${ucl?'Classifica unica Champions League':'Classifica del campionato'}" tabindex="0"><table><thead><tr><th>Squadra</th><th>PG</th><th>V</th><th>N</th><th>P</th><th>GF</th><th>GS</th><th>DR</th><th>Pt</th></tr></thead><tbody>${rows.map((r,i)=>{const team=teamById(r.id),zone=ucl?(i<8?'ucl-top8':i<24?'ucl-playoff':'ucl-out'):'';return `<tr class="${[r.id===USER_ID?'us-row':'',zone].filter(Boolean).join(' ')}" style="${teamCssVars(team)}"><td><span class="standing-team">${teamColorDot(team)}<span>${i+1}. ${esc(r.name)}</span></span></td><td>${r.p}</td><td>${r.w}</td><td>${r.d}</td><td>${r.l}</td><td>${r.gf}</td><td>${r.ga}</td><td>${r.gf-r.ga}</td><td>${r.pts}</td></tr>`}).join('')}</tbody></table></div>`}

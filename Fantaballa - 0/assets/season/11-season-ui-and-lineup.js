...
function renderSeasonTopTeam(team,{role,venue,ovr,side}){
 ...
 return `<div class="season-top-team ${esc(side||'')}" style="${teamCssVars(safeTeam)}"><span class="season-top-team-colors" aria-hidden="true"></span><div class="season-top-team-badge" aria-hidden="true">${esc(seasonTopTeamInitials(name))}</div><div class="season-top-team-copy"><small>${esc(role)} · ${esc(venue)}</small><strong title="${esc(name)}">${esc(name)}</strong><span>OVR <b>${Math.max(1,Math.round(Number(ovr)||0))}</b></span></div></div>`;
}

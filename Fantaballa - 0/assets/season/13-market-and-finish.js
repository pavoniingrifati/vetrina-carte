...
async function sendSeason(){
 const s=userStanding(),regularRank=sortedTable().findIndex(row=>row.id===USER_ID)+1,titleWon=leaguePlayoffsRuleActive()?leaguePlayoffTitleWon():regularRank===1,allowAnyFinish=Boolean(SEASON_CONFIG.submission?.allowAnyFinish);
 ...
 const payload={codice_vittoria:submissionCode,victoryCode:submissionCode,squadra:state.teamName,allenatore:state.coachName,tipo_allenatore:coachProfile().name,modalita:chaosEnabled()?SEASON_CONFIG.submission.chaosLabel:SEASON_CONFIG.submission.standardLabel,modalita_tipo:chaosEnabled()?SEASON_CONFIG.submission.chaosType:SEASON_CONFIG.submission.standardType,posizione_finale:allowAnyFinish?regularRank:1,punti:s.pts,giornate:seasonLength(),vittorie:s.w,pareggi:s.d,sconfitte:s.l,gol_fatti:s.gf,gol_subiti:s.ga,modulo:state.formation,ovr_medio:Math.round(teamPowerBase()*10)/10,capocannoniere:userScorerText,capocannoniere_giocatore:userScorerText,capocannoniere_campionato:leagueScorerText};

...
function seasonEventVisualButtonAttrs({event={},choice={},tone='blue',fallbackCategory=''}={}){
 const visual=(globalThis.SEASON_EVENT_VISUALS&&typeof globalThis.SEASON_EVENT_VISUALS.get==='function')
   ? globalThis.SEASON_EVENT_VISUALS.get({event,choice,tone,fallbackCategory})
   : {category:tone==='red'?'rischio':'campo',asset:''};
...
function renderError404StoryEvent(event){
 ...
 let choices='';
 if(event.storyType==='opening')choices=(()=>{const left=seasonEventVisualButtonAttrs({event,choice:{label:'RIAVVIA LA PARTITA',effect:'Riavvia il match e riparti.'},tone:'red',fallbackCategory:'rischio'}),right=seasonEventVisualButtonAttrs({event,choice:{label:'CONTINUA COMUNQUE',effect:'Vai avanti con la storia.'},tone:'blue',fallbackCategory:'misterioso'});return `<button class="choice season-event-choice tone-red${left.visualClass}"${left.art} data-error404-action="restart"><b>RIAVVIA LA PARTITA</b></button><button class="choice season-event-choice tone-blue${right.visualClass}"${right.art} data-error404-action="continue"><b>CONTINUA COMUNQUE</b></button>`;})();
 else if(event.storyType==='technician')choices=(()=>{const left=seasonEventVisualButtonAttrs({event,choice:{label:'INSTALLA L’ANTIVIRUS',effect:'Ascolta il tecnico di sistema.'},tone:'blue',fallbackCategory:'dirigenza'}),right=seasonEventVisualButtonAttrs({event,choice:{label:'NON DARE ASCOLTO AL TECNICO',effect:'Ignora l’avviso del tecnico.'},tone:'red',fallbackCategory:'rischio'});return `<button class="choice season-event-choice tone-blue${left.visualClass}"${left.art} data-error404-action="install"><b>INSTALLA L’ANTIVIRUS</b></button><button class="choice season-event-choice tone-red${right.visualClass}"${right.art} data-error404-action="ignore"><b>NON DARE ASCOLTO AL TECNICO</b></button>`;})();
 else choices=(()=>{const only=seasonEventVisualButtonAttrs({event,choice:{label:'RIAVVIA',effect:'Riavvia il sistema.'},tone:'red',fallbackCategory:'rischio'});return `<button class="choice season-event-choice tone-red${only.visualClass}"${only.art} data-error404-action="restart"><b>RIAVVIA</b></button>`;})();
...
function renderFantaballopoliEvent(event){
 ...
 let choices='';
 if(event.storyType==='opening')choices=(()=>{const a=seasonEventVisualButtonAttrs({event,choice:{label:'Accetta di perdere',effect:'Entra nella Triade.'},tone:'blue',fallbackCategory:'dirigenza'}),b=seasonEventVisualButtonAttrs({event,choice:{label:'Rifiuta di perdere',effect:'Combatti il sistema.'},tone:'red',fallbackCategory:'rischio'}),c=seasonEventVisualButtonAttrs({event,choice:{label:'Non voglio partecipare',effect:'Continua normalmente.'},tone:'blue',fallbackCategory:'campo'});return `<button class="choice season-event-choice tone-blue${a.visualClass}"${a.art} data-fanta-action="accept"><b>Accetta di perdere</b><small>Entra nella Triade. Il boss finale sarà L’Inter degli Onesti.</small></button><button class="choice season-event-choice tone-red${b.visualClass}"${b.art} data-fanta-action="resist"><b>Rifiuta di perdere</b><small>Combatti il sistema. Il boss finale sarà La Juve della Triade.</small></button><button class="choice season-event-choice tone-blue${c.visualClass}"${c.art} data-fanta-action="optout"><b>Non voglio partecipare</b><small>Annulla Fantaballopoli per questa stagione e continua normalmente.</small></button>`;})();
...
function renderMeritStoryEvent(event){
 ...
 if(event.storyType==='opening'){
   if(event.context?.selectingStarter){
     choices=meritCompatibleStarters().map(entry=>{const v=seasonEventVisualButtonAttrs({event,choice:{label:entry.player?.name||'Giocatore',effect:`${entry.slot} · OVR ${Number(entry.player?.ovr)||0}`},tone:'blue',fallbackCategory:'tattica'});return `<div class="season-event-choice-float"><button class="choice season-event-choice tone-blue${v.visualClass}"${v.art} data-merit-starter="${esc(entry.playerId)}" type="button"><span class="season-event-option-label">Titolare da sostituire</span><b>${esc(entry.player?.name||'Giocatore')}</b><small>${esc(entry.slot)} · OVR ${Number(entry.player?.ovr)||0}</small></button></div>`;}).join('');
   }else choices=(()=>{const a=seasonEventVisualButtonAttrs({event,choice:{label:'Mettilo in campo',effect:'Prenderà il posto di un tuo titolare.'},tone:'blue',fallbackCategory:'campo'}),b=seasonEventVisualButtonAttrs({event,choice:{label:'Sei un bidone!',effect:'Fallo marcire in panchina.'},tone:'red',fallbackCategory:'tattica'});return `<div class="season-event-choice-float"><button class="choice season-event-choice tone-blue${a.visualClass}"${a.art} data-merit-action="promote" type="button"><span class="season-event-option-label">Opzione A</span><b>Mettilo in campo</b><small>Prenderà il posto di un tuo titolare.</small></button></div><div class="season-event-choice-float"><button class="choice season-event-choice tone-red${b.visualClass}"${b.art} data-merit-action="reject" type="button"><span class="season-event-option-label">Opzione B</span><b>Sei un bidone!</b><small>Fallo marcire in panchina.</small></button></div>`;})();
 }else{
   ...
   choices=(()=>{const v=seasonEventVisualButtonAttrs({event,choice:{label:copy,effect},tone:'blue',fallbackCategory:'misterioso'});return `<div class="season-event-choice-float"><button class="choice season-event-choice tone-blue${v.visualClass}"${v.art} data-merit-ack type="button"><span class="season-event-option-label">Storia</span><b>${copy}</b><small>${effect}</small></button></div>`;})();
 }

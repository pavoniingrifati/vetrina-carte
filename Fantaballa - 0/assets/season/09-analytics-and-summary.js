...
function seasonRuleSummary(){
 ...
 if(typeof buffonGlovesActive==='function'&&buffonGlovesActive())extras.push('Guanti di Buffon: massimo 2 gol subiti in ogni partita');
 ...
 if(typeof presidentChallengeProgressLabel==='function'){const presidentLabel=presidentChallengeProgressLabel();if(presidentLabel)extras.push(presidentLabel)}
...
function resolveLineup(){if(typeof preparePresidentChallengeBeforeLineup==='function')preparePresidentChallengeBeforeLineup();enforceTipsterStarters();if(typeof enforceCaptainArmbandStarter==='function')enforceCaptainArmbandStarter();if(state.seasonRules?.autoOptimizeLineup)optimizeLineupWithBench();if(typeof enforceCaptainArmbandStarter==='function')enforceCaptainArmbandStarter();if(typeof enforcePresidentChallengeLineup==='function')enforcePresidentChallengeLineup();enforceFrenchFlyingPositions();if(typeof enforceBadLuckPlayerLineup==='function')enforceBadLuckPlayerLineup();if(typeof enforcePresidentChallengeLineup==='function')enforcePresidentChallengeLineup();return applyWrongShirtSwap(resolveRosterLineup(rosterPlayers()))}
...
function coachTrailingNonWinCount(){const history=Array.isArray(state?.history)?state.history:[];let count=0;for(let index=history.length-1;index>=0;index--){const item=history[index];if(Number(item.gf)>Number(item.ga))break;count++}return count}
function coachMatchOvrModifier(){if(!coachIs('motivator'))return 0;const streak=coachTrailingStreak(),bonusesDisabled=typeof noMisterCoachBonusesDisabled==='function'&&noMisterCoachBonusesDisabled();if(coachTrailingNonWinCount()>=2)return bonusesDisabled?0:3;if(streak.type==='win'&&streak.count===3)return-3;return 0}
function coachCurrentEffectLabel(){if(typeof noMisterCoachBonusesDisabled==='function'&&noMisterCoachBonusesDisabled())return'Partita senza mister: bonus dell’allenatore disattivati per la prossima gara';if(coachIs('motivator')){const value=coachMatchOvrModifier(),boost='bonus OVR/Intesa potenziati di +2 OVR e +2 Intesa';return value>0?`Motivatore: +3 OVR nella prossima partita · ${boost}`:value<0?`Motivatore: -3 OVR nella prossima partita · ${boost}`:`Motivatore: ${boost}`}if(coachIs('salvation')){const value=coachRosterAverageOvr();return value<70?`Mister salvezza: +1 gol aggiuntivo a ogni partita (OVR rosa ${value.toFixed(1)})`:value>80?`Mister salvezza: gol subito garantito (OVR rosa ${value.toFixed(1)})`:`Mister salvezza: fascia neutra (OVR rosa ${value.toFixed(1)})`}if(coachIs('talent-scout'))return'Talent scout: primo pack dalla squadra dell’omonimo, scouting potenziato e mercato esterno bloccato';if(coachIs('young-beautiful'))return'Giovani e belli: +20 Intesa agli OVR base 60–69, +10 agli OVR base 70–75, vietati gli OVR base da 85 in su e 0 re-roll nel draft iniziale';if(coachIs('ductility'))return'Duttilità: nessun malus fuori ruolo, Intesa sempre 0, nessun altro bonus OVR; chi segna fuori ruolo guadagna +1 OVR fino a fine stagione';if(coachIs('three-five-two'))return'3-5-2: rosa di 14 giocatori generata dal primo club del draft, modulo bloccato sul 3-5-2, probabilità eventi dimezzata e nessun draft di metà stagione';if(coachIs('rokky'))return'Rokky: puoi tentare di annullare un gol avversario (80% successo, 20% azzeramento punti), inizi con un Fischietto di Collina e gli avversari hanno +5 OVR contro di te';return'Anonimo: nessun effetto'}
...
function journeyScore(match){
 const gf=Number.isFinite(Number(match?.displayGf))?Number(match.displayGf):Number(match?.gf)||0;
 const ga=Number.isFinite(Number(match?.displayGa))?Number(match.displayGa):Number(match?.ga)||0;
 return{gf,ga,label:`${gf}–${ga}${match?.penalties?' d.c.r.':''}`};
}
function journeyOutcome(match){const score=journeyScore(match);return score.gf>score.ga?'win':score.gf<score.ga?'loss':'draw'}
function journeyOutcomeLabel(match){const type=journeyOutcome(match);return type==='win'?'Vittoria':type==='loss'?'Sconfitta':'Pareggio'}
function journeyForm(limit=5){return (Array.isArray(state.history)?state.history:[]).slice(-Math.max(1,Number(limit)||5)).map(match=>({type:journeyOutcome(match),label:journeyOutcome(match)==='win'?'V':journeyOutcome(match)==='loss'?'S':'P'}))}
function journeyScorers(match){
 const names=(Array.isArray(match?.goals)?match.goals:[]).map(goal=>String(goal?.playerName||goal?.scorer||statPlayerInfo(goal?.playerId)?.name||'')).filter(Boolean);
 if(!names.length)return'';
 const counts={};names.forEach(name=>counts[name]=(counts[name]||0)+1);
 return Object.entries(counts).slice(0,3).map(([name,count])=>`${name}${count>1?` ×${count}`:''}`).join(' · ');
}
function journeyMvp(match){const id=String(match?.mvpId||'');if(!id)return'';const player=statPlayerInfo(id);return String(player?.name||'')}
function renderJourneyMatchRows(){
 const history=Array.isArray(state.history)?state.history:[];
 if(!history.length)return'<p class="journey-empty">Nessuna partita disputata.</p>';
 return [...history].reverse().map(match=>{const score=journeyScore(match),outcome=journeyOutcome(match),scorers=journeyScorers(match),mvp=journeyMvp(match),details=[match.home?'Casa':'Trasferta',scorers?`Gol: ${scorers}`:'',mvp?`MVP: ${mvp}`:''].filter(Boolean).join(' · ');return `<article class="journey-row ${outcome}"><div class="journey-day"><small>G</small><b>${Math.max(1,Number(match.matchday)||1)}</b></div><div class="journey-row-main"><div class="journey-row-head"><b>${esc(match.opponent||'Avversario')}</b><span class="journey-score">${esc(score.label)}</span></div><small>${esc(details)}</small></div><span class="journey-outcome">${journeyOutcomeLabel(match)}</span></article>`}).join('');
}
function renderJourneyEventRows(){
 const logs=Array.isArray(ensureSeasonAnalytics().eventLog)?ensureSeasonAnalytics().eventLog:[];
 if(!logs.length)return'<p class="journey-empty">Nessun evento registrato.</p>';
 return [...logs].reverse().map(item=>{const score=Number(item?.score)||0,impact=score>0?'positive':score<0?'negative':'neutral',choice=String(item?.choice||'').trim(),result=String(item?.result||item?.effect||'').trim(),kind=item?.automatic?'Automatico':item?.kind==='decision'?'Decisione':'Evento';return `<article class="journey-event-row ${impact}"><div class="journey-day"><small>G</small><b>${Math.max(1,Number(item?.matchday)||1)}</b></div><div class="journey-row-main"><div class="journey-row-head"><b>${esc(item?.title||'Evento')}</b><span class="journey-event-kind">${esc(kind)}</span></div>${choice?`<span class="journey-choice">→ ${esc(choice)}</span>`:''}${result?`<small>${esc(result)}</small>`:''}</div></article>`}).join('');
}
function renderSeasonMatchHistoryDetails(){
 const count=Array.isArray(state.history)?state.history.length:0;
 return `<details class="journey-details"><summary><span>📅 Storico partite</span><em>${count}</em></summary><div class="journey-list">${renderJourneyMatchRows()}</div></details>`;
}
function renderSeasonJourney(){
 const history=Array.isArray(state.history)?state.history:[],events=Array.isArray(ensureSeasonAnalytics().eventLog)?ensureSeasonAnalytics().eventLog:[],last=history[history.length-1]||null,standing=userStanding()||{w:0,d:0,l:0},form=journeyForm(),activeCount=(Array.isArray(state.activeEffects)?state.activeEffects.length:0)+(state.quest?.active?1:0),lastScore=last?journeyScore(last):null,lastScorers=last?journeyScorers(last):'',lastMvp=last?journeyMvp(last):'';
 return `<section class="season-journey"><div class="journey-overview"><div><span>Bilancio</span><b>${Number(standing.w)||0}-${Number(standing.d)||0}-${Number(standing.l)||0}</b></div><div><span>Forma</span><b class="journey-form">${form.length?form.map(item=>`<i class="${item.type}">${item.label}</i>`).join(''):'—'}</b></div><div><span>Eventi</span><b>${events.length}</b></div><div><span>Effetti attivi</span><b>${activeCount}</b></div></div>${last?`<article class="journey-last ${journeyOutcome(last)}"><div><span>Ultima partita · Giornata ${Math.max(1,Number(last.matchday)||history.length)}</span><b>${esc(state.teamName)} ${esc(lastScore.label)} ${esc(last.opponent||'Avversario')}</b><small>${[last.home?'In casa':'In trasferta',lastScorers?`Gol: ${lastScorers}`:'',lastMvp?`MVP: ${lastMvp}`:''].filter(Boolean).map(esc).join(' · ')}</small></div><strong>${journeyOutcomeLabel(last)}</strong></article>`:'<p class="journey-empty journey-empty-main">Il percorso inizierà dopo la prima partita.</p>'}${renderSeasonMatchHistoryDetails()}<details class="journey-details"><summary><span>🧭 Eventi e decisioni</span><em>${events.length}</em></summary><div class="journey-list">${renderJourneyEventRows()}</div></details></section>`;
}

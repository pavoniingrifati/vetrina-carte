...
function chemistryBaseRaw(player,list){if(coachIs('ductility'))return 0;const players=Array.isArray(list)?list:[];let v=nationChemistryBonus(player,players)+clubChemistryBonus(player,players)+youngBeautifulChemistryBonus(player);if(isSubscriber(player))v+=5;const nationKey=chemistryNationKey(player),subs=nationKey?players.filter(o=>chemistryNationKey(o)===nationKey&&isSubscriber(o)).length:0;if(isSubscriber(player)&&nationKey&&subs>=2)v+=10;if(!(typeof noMisterCoachBonusesDisabled==='function'&&noMisterCoachBonusesDisabled())&&normalizeName(player.name)===normalizeName(state.coachName))v+=10;v+=motivatorPermanentChemistryBonus(player);return v}

...
function renderActiveQuest(){
 ...
 return `<div class="quest-card fanta-quest-card"><div class="quest-card-head"><div class="quest-card-badge">🎯</div><div class="quest-card-title"><span>Quest attiva</span><b>${esc(q.title)}</b><small>${q.reward?`Premio · ${esc(q.reward)}`:''}</small></div><div class="quest-card-deadline">${q.deadlineMatches?`<strong>${Math.max(0,q.deadlineMatches-q.matchesPlayed)}</strong><span>giornate</span>`:'<strong>∞</strong><span>tempo</span>'}</div></div><p>${esc(q.objective)}</p><div class="quest-progress"><span style="width:${Math.round(clamp(ratio,0,1)*100)}%"></span></div><div class="quest-progress-copy"><strong>${esc(questProgressText(q))}</strong>${q.deadlineMatches?`<span>${Math.max(0,q.deadlineMatches-q.matchesPlayed)} giornate rimaste</span>`:''}</div><div class="quest-stakes"><small><b>Successo:</b> ${esc(q.reward)}</small><small><b>Fallimento:</b> ${esc(q.penalty)}</small></div>${q.notice?`<div class="quest-notice">${esc(q.notice)}</div>`:''}${typeof renderLeaderQuestSelection==='function'?renderLeaderQuestSelection(q):''}</div>`;
}

...
function versionedResourceUrl(url){const value=String(url||'');if(!value||/^(?:https?:|data:|blob:)/i.test(value))return value;const sep=value.includes('?')?'&':'?';return `${value}${sep}v=20260722-3`}
...
function normalizeCampionatoState(input){
 ...
 next.quest.scorerIds=[...new Set((Array.isArray(next.quest.scorerIds)?next.quest.scorerIds:[]).map(String).filter(Boolean))];next.quest.scorerNames=next.quest.scorerNames&&typeof next.quest.scorerNames==='object'?next.quest.scorerNames:{};

...
function playerAcquisitionBlocked(){return Boolean(state?.seasonRules?.marketBlocked)}
function playerAcquisitionBlockMessage(){return 'Mercato bloccato: non puoi più ricevere nuovi giocatori fino al termine della stagione.'}
function playerArrivalIsBlocked(){return playerAcquisitionBlocked()||talentScoutBlocksExternalArrival()}
function playerArrivalBlockMessage(){return playerAcquisitionBlocked()?playerAcquisitionBlockMessage():talentScoutBlockMessage()}
...
function phaseLabel(value){return({setup:'Impostazione',draft:'Draft iniziale',season:isChampionsCompetition()?'Fase campionato Champions':'Campionato',midseason:'Mercato di metà stagione','champions-knockout':'Fase a eliminazione Champions',finished:'Stagione conclusa'})[value]||'Stagione'}
...
function versionedResourceUrl(url){return String(url||'')}
...
function updateCompetitionChrome(){
 const profile=competitionVariantProfile(state?.competitionVariant),legend=profile.id==='legend',champions=profile.id==='champions';
 document.body?.classList.toggle('competition-legend',legend);document.body?.classList.toggle('competition-champions',champions);
 const meta=document.getElementById('competitionHeaderMeta');if(meta)meta.textContent=champions?'Champions League · 36 squadre · 4 fasce · 8 giornate · playoff + fase a eliminazione':legend?'Legend · 38 giornate · 20 squadre per run · 40 club disponibili · 796 campioni · OVR fino a 116':'Serie A · 38 giornate · 20 club · 531 calciatori reali';
 const hero=document.getElementById('competitionHeroText');if(hero)hero.textContent=champions?'Il formato Champions League attuale dentro Fantaballa: 36 squadre, sorteggio per fasce, otto avversarie diverse, classifica unica e fase a eliminazione diretta fino alla finale.':legend?'Le squadre che hanno fatto la storia del calcio mondiale dentro il Fantacampionato: draft tra 40 club leggendari, 38 giornate, eventi, infortuni, squalifiche e mercato di metà stagione.':'I calciatori e i club della stagione 2026/2027 dentro il Campionato di Fantaballa: draft, 38 giornate, eventi, infortuni, squalifiche e mercato di metà stagione.';
}
...
function seasonLength(source=state){const scheduled=Array.isArray(source?.schedule)?source.schedule.length:0;if(String(source?.competitionVariant||'')==='champions')return Math.max(8,scheduled);const configured=Math.max(1,Number(source?.seasonRules?.seasonLength)||38);return source?.seasonRules?.dynamicLeague?Math.max(configured,scheduled):Math.max(38,configured,scheduled)}
...
function normalizeCampionatoState(input){
 ...
 if(Object.prototype.hasOwnProperty.call(next.draft,'lastPlacement'))delete next.draft.lastPlacement;
 ...
 next.seasonRules.stadiumHomeAdvantageBonus=clamp(Number(next.seasonRules.stadiumHomeAdvantageBonus)||0,0,.15);
 ...
 next.seasonRules.figcIncidentRule=['negative','positive'].includes(String(next.seasonRules.figcIncidentRule))?String(next.seasonRules.figcIncidentRule):'';
 ...
 next.seasonRules.fgciDirectMatchRule=['penalty-lottery','effective-time'].includes(String(next.seasonRules.fgciDirectMatchRule))?String(next.seasonRules.fgciDirectMatchRule):'';
 next.seasonRules.heroAcademiaRule=String(next.seasonRules.heroAcademiaRule)==='four-one'?'four-one':'';
 next.seasonRules.heroAcademiaPortalUsed=Boolean(next.seasonRules.heroAcademiaPortalUsed);
 next.seasonRules.heroAcademiaPortalDestination=String(next.seasonRules.heroAcademiaPortalDestination||'');
 ...
 next.seasonRules.marketBlocked=Boolean(next.seasonRules.marketBlocked);
 next.seasonRules.bottomHalfUnbeaten=Boolean(next.seasonRules.bottomHalfUnbeaten);
 next.seasonRules.thriftyPresidentRemovedPlayers=[...new Set((Array.isArray(next.seasonRules.thriftyPresidentRemovedPlayers)?next.seasonRules.thriftyPresidentRemovedPlayers:[]).map(String).filter(Boolean))];
 ...
 const fantaDefaults=freshState().story.fantaballopoli;
 const legacyFanta=next.story.fantaballopoli&&typeof next.story.fantaballopoli==='object'?next.story.fantaballopoli:{};
 const legacyStage=String(legacyFanta.stage||'idle');
 const legacyStageMap={forced_loss_pending:'evil_forced_loss',part2_waiting:'evil_player_waiting',curse:'evil_boss_waiting',satisfaction_waiting:'evil_boss_waiting',corruption:'evil_boss_waiting',investigator_waiting:'evil_boss_waiting',accusation:'evil_boss_waiting',confessed:'evil_boss_waiting',ended_denial_no_title:'evil_boss_waiting',juventus_final:'boss_final'};
 const normalizedStage=legacyStageMap[legacyStage]||legacyStage;
 const inferredPath=['evil_forced_loss','evil_player_waiting','awaiting_midseason','evil_waiting_midseason','evil_trials_waiting','evil_no_giuda_waiting','evil_boss_waiting'].includes(normalizedStage)||legacyFanta.giudaId||legacyFanta.curseActive?'evil':['good_witness_waiting','good_boss_waiting','boss_final'].includes(normalizedStage)&&legacyStage==='juventus_final'?'resistance':'';
 const normalizedPath=['evil','resistance','none'].includes(String(legacyFanta.path||''))?String(legacyFanta.path):inferredPath;
 const fantaStoryState={...fantaDefaults,...legacyFanta,stage:normalizedStage,path:normalizedPath,cartonati:{...fantaDefaults.cartonati,...(legacyFanta.cartonati&&typeof legacyFanta.cartonati==='object'?legacyFanta.cartonati:{})},intercettazioni:{...fantaDefaults.intercettazioni,...(legacyFanta.intercettazioni&&typeof legacyFanta.intercettazioni==='object'?legacyFanta.intercettazioni:{})},challenge:{...fantaDefaults.challenge,...(legacyFanta.challenge&&typeof legacyFanta.challenge==='object'?legacyFanta.challenge:{})},finale:{...fantaDefaults.finale,...(legacyFanta.finale&&typeof legacyFanta.finale==='object'?legacyFanta.finale:{})}};
 ...
 fantaStoryState.path=['evil','resistance','none'].includes(String(fantaStoryState.path||''))?String(fantaStoryState.path):'';
 ...
 fantaStoryState.targetTradeOrdered=Boolean(fantaStoryState.targetTradeOrdered||((fantaStoryState.stage==='awaiting_midseason'||fantaStoryState.stage==='evil_trials_waiting')&&fantaStoryState.targetPlayerId));
 fantaStoryState.targetTraded=Boolean(fantaStoryState.targetTraded&&fantaStoryState.stage!=='awaiting_midseason');
 fantaStoryState.targetLocked=Boolean(fantaStoryState.targetLocked);
 ...
 fantaStoryState.giudaLost=Boolean(fantaStoryState.giudaLost);
 fantaStoryState.giudaLostReason=String(fantaStoryState.giudaLostReason||'');
 fantaStoryState.giudaCurseWins=Math.max(0,Number(fantaStoryState.giudaCurseWins)||0);
 fantaStoryState.giudaCurseTotal=Math.max(0,Number(fantaStoryState.giudaCurseTotal)||fantaStoryState.giudaCurseWins*5);
 fantaStoryState.power=Math.max(0,Number(fantaStoryState.power)||0);
 fantaStoryState.suspicion=Math.max(0,Number(fantaStoryState.suspicion)||0);
 fantaStoryState.completed=Boolean(fantaStoryState.completed);
 for(const key of ['arbitro','giuda','silenzio'])fantaStoryState.cartonati[key]=Boolean(fantaStoryState.cartonati[key]);
 for(const key of ['arbitro','testimone','triade'])fantaStoryState.intercettazioni[key]=Boolean(fantaStoryState.intercettazioni[key]);
 fantaStoryState.challenge.id=String(fantaStoryState.challenge.id||'');
 fantaStoryState.challenge.active=Boolean(fantaStoryState.challenge.active);
 fantaStoryState.challenge.status=String(fantaStoryState.challenge.status||'idle');
 fantaStoryState.challenge.matchesPlayed=Math.max(0,Number(fantaStoryState.challenge.matchesPlayed)||0);
 fantaStoryState.challenge.matchesRequired=Math.max(0,Number(fantaStoryState.challenge.matchesRequired)||0);
 fantaStoryState.challenge.progress=Math.max(0,Number(fantaStoryState.challenge.progress)||0);
 fantaStoryState.challenge.wins=Math.max(0,Number(fantaStoryState.challenge.wins)||0);
 fantaStoryState.challenge.cleanSheets=Math.max(0,Number(fantaStoryState.challenge.cleanSheets)||0);
 fantaStoryState.challenge.goals=Math.max(0,Number(fantaStoryState.challenge.goals)||0);
 fantaStoryState.challenge.scored=Boolean(fantaStoryState.challenge.scored);
 fantaStoryState.challenge.lastMatchday=Number.isFinite(Number(fantaStoryState.challenge.lastMatchday))?Number(fantaStoryState.challenge.lastMatchday):-1;
 fantaStoryState.challenge.resultText=String(fantaStoryState.challenge.resultText||'');
 fantaStoryState.challenge.nextId=String(fantaStoryState.challenge.nextId||'');
 fantaStoryState.lastChallengeResult=fantaStoryState.lastChallengeResult&&typeof fantaStoryState.lastChallengeResult==='object'?fantaStoryState.lastChallengeResult:null;
 fantaStoryState.finale.eligible=Boolean(fantaStoryState.finale.eligible);
 fantaStoryState.finale.bossId=String(fantaStoryState.finale.bossId||'');
 fantaStoryState.finale.bossName=String(fantaStoryState.finale.bossName||'');
 ...
 fantaStoryState.finale.extraTime=Boolean(fantaStoryState.finale.extraTime);
 fantaStoryState.finale.penalties=fantaStoryState.finale.penalties&&typeof fantaStoryState.finale.penalties==='object'?fantaStoryState.finale.penalties:null;
 fantaStoryState.finale.notes=Array.isArray(fantaStoryState.finale.notes)?fantaStoryState.finale.notes.map(String):[];
 fantaStoryState.finale.userScorers=Array.isArray(fantaStoryState.finale.userScorers)?fantaStoryState.finale.userScorers.map(String):[];
 fantaStoryState.finale.opponentScorers=Array.isArray(fantaStoryState.finale.opponentScorers)?fantaStoryState.finale.opponentScorers.map(String):[];
 fantaStoryState.finale.activePowers=Array.isArray(fantaStoryState.finale.activePowers)?fantaStoryState.finale.activePowers.map(String):[];
 ...
 fantaStoryState.finale.pointsAfterBonus=Number(fantaStoryState.finale.pointsAfterBonus)||0;
 next.story.fantaballopoli=fantaStoryState;
 ...
 next.champions=next.champions&&typeof next.champions==='object'?next.champions:{};
 next.champions.version=1;
 next.champions.replacedPot4ClubId=String(next.champions.replacedPot4ClubId||'');
 next.champions.leagueRank=clamp(Math.floor(Number(next.champions.leagueRank)||0),0,36);
 next.champions.leagueTable=Array.isArray(next.champions.leagueTable)?next.champions.leagueTable:[];
 next.champions.stage=['league','playoff','round16','quarter','semi','final','completed'].includes(String(next.champions.stage))?String(next.champions.stage):'league';
 next.champions.stageIndex=Math.max(0,Math.floor(Number(next.champions.stageIndex)||0));
 next.champions.ties=Array.isArray(next.champions.ties)?next.champions.ties:[];
 next.champions.history=Array.isArray(next.champions.history)?next.champions.history:[];
 next.champions.championId=String(next.champions.championId||'');
 next.champions.userEliminated=Boolean(next.champions.userEliminated);
 next.champions.userExitStage=String(next.champions.userExitStage||'');
 ...
 if(!['setup','draft','season','midseason','story-final','italia-2006-final','fantaballopoli-final','fantaballopoli-restart','playoffs','champions-knockout','finished'].includes(next.phase))next.phase='setup';
 next.schedule=Array.isArray(next.schedule)?next.schedule:[];
 if(!isChampionsCompetition(next)&&next.teams.length&&(next.seasonRules.marathon||next.seasonRules.dynamicLeague)){

...
function repairState(){
 ...
 state.influence=previousVersion<8?clamp(oldInfluence+(INFLUENCE_START-LEGACY_INFLUENCE_START),0,INFLUENCE_BONUS_CAP):clamp(oldInfluence,0,INFLUENCE_BONUS_CAP);
...
function refereeAssignmentDescription(assignment){
 ...
 const match=home&&away?`${home.name}–${away.name}`:'partita scelta',verb=referee?.chooseDisfavored?'sfavorisce':'favorisce',reward=assignment.selfDisadvantageReward?` · +${SELF_DISADVANTAGE_REWARD} Influenza per autosabotaggio`:'';
 return `${referee?.title||'Arbitro'} · ${match}${selected?` · ${verb} ${selected.name}`:''}${reward}`;
}
function selfDisadvantageRewardUsedThisRound(){return (state?.refereeHistory||[]).some(item=>Number(item?.round)===Number(state?.round)+1&&item?.selfDisadvantageReward)}
function earnsSelfDisadvantageInfluence(referee,selectedTeamId){return Boolean(referee?.chooseDisfavored&&String(selectedTeamId||'')===String(state?.targetTeamId||'')&&!selfDisadvantageRewardUsedThisRound())}
...
function renderMissionHero(){
 ...
 const dots=Array.from({length:INFLUENCE_START},(_,index)=>`<span class="influence-dot ${index<Math.min(state.influence,INFLUENCE_START)?'active':''}"></span>`).join(''),bonusInfluence=Math.max(0,(Number(state.influence)||0)-INFLUENCE_START);
 return `<section class="mission-hero" style="${teamStyle(team)}"><div class="mission-grid"><div><div class="mission-head"><div class="mission-mini-crest">${esc(team.shortName)}</div><div class="mission-title"><div class="label" style="color:#ffe96c">Missione stagionale</div><h1>Porta ${esc(team.name)} al titolo</h1><p>Non gestisci la rosa: governi regolamenti e designazioni arbitrali.</p></div></div><div class="influence-meter"><div class="influence-head"><span>Influenza Federale disponibile</span><b>${state.influence}${bonusInfluence?` · +${bonusInfluence} bonus`:''}</b></div><div class="influence-dots">${dots}</div></div></div><div class="mission-stats"><div class="mission-stat"><b>${rank||'—'}°</b><span>Posizione</span></div><div class="mission-stat"><b>${standing.pts}</b><span>Punti</span></div><div class="mission-stat"><b>${gap===0?'0':gap}</b><span>Dalla vetta</span></div><div class="mission-stat"><b>${active.length}</b><span>Regole attive</span></div><div class="mission-stat"><b>${esc(directorDifficultyProfile().label)}</b><span>Difficoltà</span></div></div></div></section>`;
}
...
function assignRoundReferee(id,fixture=null,favoredTeamId=''){
 ...
 if(selfDisadvantageReward)state.influence=clamp((Number(state.influence)||0)+SELF_DISADVANTAGE_REWARD,0,INFLUENCE_BONUS_CAP);
 ...
 const rewardBanner=selfDisadvantageReward?`<div class="self-disadvantage-reward"><b>+${SELF_DISADVANTAGE_REWARD} Influenze</b><span>Hai scelto di sfavorire ${esc(targetTeam()?.name||'la tua squadra')}: dopo il costo della designazione guadagni 1 Influenza netta. Ora hai ${state.influence} Influenze disponibili.</span></div>`:'';
 ...
 if(selfDisadvantageReward)toast(`Autosabotaggio: +${SELF_DISADVANTAGE_REWARD} Influenze, guadagno netto +1. Totale ${state.influence}.`);
}

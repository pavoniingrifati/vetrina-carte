...
function refereeAssignmentDescription(assignment){
 ...
 const match=home&&away?`${home.name}–${away.name}`:'partita scelta',verb=referee?.chooseDisfavored?'sfavorisce':'favorisce';
 return `${referee?.title||'Arbitro'} · ${match}${selected?` · ${verb} ${selected.name}`:''}`;
}
...
function applyDirectorOvrRegulations(goals,home,away,duration,commentary){
 ...
 if(weaker&&hasRule('underdogGoalChance')&&rand()<.35){const goal=makeRefereeGoal(weaker.id,randomInt(2,Math.max(2,duration)),pickScorer(weaker.id));goal.refereeGoal=false;goal.regulationGoal=true;goals.push(goal);commentary.push({minute:`${goal.minute}’`,text:`La fionda di Davide: gol bonus per ${weaker.name}.`,type:'rule'})}

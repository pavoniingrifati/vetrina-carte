...
test('Nuova decisione comune consentita', () => {
  ...
  catalogs[0].decisions.push({id:'evento-editor-test',order:208,title:'Evento editor test',text:'Evento generato dal test.',choices:[{label:'A',effect:'A',applyHandler:'nuovo-sponsor:0'},{label:'B',effect:'B',applyHandler:'nuovo-sponsor:1'}]});
  const report=validate(catalogs);
  assert(report.ok, report.errors.join(' | '));
  assert(report.stats.communityDecisions===90 && report.stats.realDecisions===84,'Conteggi nuova decisione errati.');
});

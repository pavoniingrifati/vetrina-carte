...
const posPatterns={P:['P'],D:['DC','TS','TD','DC','DC, TS','DC, TD','TS, TD'],C:['CC','CDC','COC','CC, CDC','CC, COC','CC'],A:['ATT','AS','AD','AS, ATT','AD, ATT','ATT, COC','AS, COC','AD, COC']};
const star={
...
function choosePos(role,index,name){const list=posPatterns[role];return list[(index+(hashInt(name)%list.length))%list.length]}
...
for(const club of source.clubs){for(const role of ['P','D','C','A']){club.players[role].forEach((raw,index)=>{const real=bestRealMatch(club.id,raw.name);let id=`ucl-${club.id}-${slug(raw.name)}`;let suffix=2;while(ids.has(id))id=`ucl-${club.id}-${slug(raw.name)}-${suffix++}`;ids.add(id);let player;if(real){reused++;player={...real,id,name:raw.name,club:club.id,uefaNat:raw.nat,ovrSource:'fantaballa-real-2026-27'};}else{const Position=choosePos(role,index,raw.name),ovr=generatedOvr(club,role,index,raw.name);player={id,name:raw.name,role,Position,roleLabel:roleLabel[role],nation:natNames[raw.nat]||raw.nat||'Non indicata',ovr,subscriber:'no',abbonato:'no',club:club.id,uefaNat:raw.nat,ovrSource:star[raw.name]!=null?'fantaballa-star-calibration':'fantaballa-ucl-calibration'};}players.push(player)})}}
